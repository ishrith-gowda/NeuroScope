...
class PairedDataset(BaseDataset):
            ...
            for source_path in sorted(source_dir.iterdir()):
                if any(source_path.name.endswith(ext) for ext in self.extensions):
                    ...
                    if target_path.exists():
                        samples.append(
                            {"source": source_path, "target": target_path, "id": source_path.stem}
                        )
    ...
    def _load_item(self, index: int) -> dict[str, Any]:
        ...
        return {"source": source, "target": target, "id": sample["id"]}
        ...
        if domain_dir.exists():
            for path in sorted(domain_dir.iterdir()):
                if any(path.name.endswith(ext) for ext in self.extensions):
                    samples.append({"path": path, "id": path.stem})
...
class BraTSDataset(BaseDataset):
    ...
    def _load_item(self, index: int) -> dict[str, Any]:
        ...
        item = {"image": torch.from_numpy(image), "subject_id": subject_id, "slice_idx": slice_idx}
...
class MultiModalMRIDataset(BaseDataset):
    ...
    def _load_samples(self) -> list[dict[str, Any]]:
        ...
        first_modality = next(iter(self.modality_dirs.keys()))
        ...
        if first_dir.exists():
            for path in sorted(first_dir.glob(self.file_pattern)):
                ...
                if valid:
                    samples.append({"id": sample_id, "modality_paths": modality_paths})
    ...
    def _load_item(self, index: int) -> dict[str, Any]:
        ...
        modality_data = []
        for modality in self.modality_dirs:
            path = sample["modality_paths"][modality]
            data = np.load(path)
        ...
        item = {"image": torch.from_numpy(image), "id": sample["id"]}
...
class DomainAdaptationDataset(Dataset):
    ...
    def __init__(self, source_dataset: Dataset, target_dataset: Dataset, paired: bool = False):
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        return {"source": source_item, "target": target_item}
...
class CycleGANDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        return {"real_A": real_a, "real_B": real_b, "idx_A": idx_a, "idx_B": idx_b}


def create_dataset(dataset_type: str, root_dir: Union[str, Path], **kwargs) -> Dataset:
    """
