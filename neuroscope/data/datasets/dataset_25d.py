        ...
        for subj_dir in sorted(self.root_dir.iterdir()):
            if subj_dir.is_dir():
                ...
                has_all = all(
                    ...
                    or (subj_dir / f"{subj_dir.name}_{mod}.nii.gz").exists()
...
def create_dataloaders(
    ...
    full_dataset = UnpairedMRIDataset25D(
        domain_a_dir=brats_dir, domain_b_dir=upenn_dir, image_size=image_size
    )
...
if __name__ == "__main__":
    ...
    dataset = UnpairedMRIDataset25D(
        domain_a_dir=brats_dir, domain_b_dir=upenn_dir, image_size=(128, 128)
    )
