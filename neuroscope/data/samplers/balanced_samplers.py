        ...
        for cls in self.classes:
            ...
            if self.replacement or len(class_indices) >= self.samples_per_class:
                sampled = np.random.choice(
                    class_indices, size=self.samples_per_class, replace=self.replacement
                )
            else:
...
class WeightedRandomSampler(Sampler[int]):
    ...
    @classmethod
    def from_labels(
        cls, labels: Sequence[int], num_samples: Optional[int] = None
...
class DomainBalancedSampler(Sampler[int]):
    ...
    def __init__(
        ...
        self.domain_sizes = {domain: len(dataset) for domain, dataset in domain_datasets.items()}
        ...
        if strategy == "uniform":
            self.domain_weights = dict.fromkeys(self.domains, 1.0)
        elif strategy == "proportional":
            total = sum(self.domain_sizes.values())
            self.domain_weights = {d: size / total for d, size in self.domain_sizes.items()}
        ...
        for domain in self.domains:
            ...
            if domain_size >= self.samples_per_domain:
                domain_indices = np.random.choice(
                    domain_size, size=self.samples_per_domain, replace=False
                )
            else:
                domain_indices = np.random.choice(
                    domain_size, size=self.samples_per_domain, replace=True
                )

            # convert to global indices
            offset = sum(self.domain_sizes[d] for d in self.domains[: self.domains.index(domain)])
...
class StratifiedSampler(Sampler[int]):
    ...
    def __init__(
        self, labels: Sequence[int], batch_size: int, shuffle: bool = True, drop_last: bool = False
    ):
        """
        args:
            labels: class label for each sample
        ...
        self.class_batch_sizes = {
            cls: max(1, round(batch_size * count / total)) for cls, count in class_counts.items()
        }
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        class_iters = {cls: iter(self.class_indices[cls]) for cls in self.classes}
...
class SubsetRandomSampler(Sampler[int]):
    ...
    def __init__(self, indices: Sequence[int], generator: Optional[torch.Generator] = None):
        """
        args:
    ...
    @classmethod
    def random_split(
        cls, dataset_size: int, train_ratio: float = 0.8, seed: Optional[int] = None
    ) -> tuple:
        """
