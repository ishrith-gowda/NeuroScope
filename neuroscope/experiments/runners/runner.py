...
class ExperimentRunner:
    ...
    def __init__(
        self, config: Any, output_dir: Union[str, Path], resume_from: Optional[str] = None
    ):
        """
        args:
    ...
    def _build_models(self) -> dict[str, nn.Module]:
        ...
        return {"G_A2B": G_A2B, "G_B2A": G_B2A, "D_A": D_A, "D_B": D_B}
    ...
    def run(self) -> dict:
        ...
        # training loop
        for epoch in range(self.trainer.start_epoch, self.config.training.epochs):
            # train epoch
            train_metrics = self.trainer.train_epoch(self.dataloaders["train"], epoch)

            self.logger.log_metrics(train_metrics, epoch, prefix="train")

            # validation
            if epoch % self.config.training.val_every == 0:
                val_metrics = self.trainer.validate(self.dataloaders["val"], epoch)
    ...
    def _final_evaluation(self) -> dict:
        ...
        test_metrics = self.trainer.evaluate(
            self.dataloaders["test"], metric_calculator=metric_calc
        )
...
class AblationRunner:
    ...
    def run(self) -> dict:
        ...
        baseline_runner = ExperimentRunner(self.base_config, self.output_dir / "baseline")
        ...
        for config_path in self.ablation_configs:
            ...
            runner = ExperimentRunner(config, self.output_dir / config.name)
            runner.setup()
    ...
    def _generate_report(self):
        ...
        report = {"experiments": list(self.results.keys()), "metrics": {}, "summary": {}}
...
class BaselineRunner:
    ...
    def __init__(self, data_config: Any, output_dir: Union[str, Path]):
        """
        args:
