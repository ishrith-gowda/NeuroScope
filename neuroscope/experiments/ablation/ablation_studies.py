...
class AblationStudy:
    ...
    def __init__(self, base_config: Any, output_dir: str, n_trials: int = 3):
        """
        args:
    ...
    def run(self, runner_class=None) -> Dict:
        ...
        for trial in range(self.n_trials):
            ...
            runner = runner_class(config, self.output_dir / "baseline" / f"trial_{trial}")
            runner.setup()
            result = runner.run()
        ...
        for ablation in self.ablations:
            ...
            for trial in range(self.n_trials):
                config = self.apply_modifications(self.base_config, ablation.modifications)
                config = self._modify_for_trial(config, trial)

                runner = runner_class(config, self.output_dir / ablation.name / f"trial_{trial}")
                runner.setup()
                result = runner.run()
        ...
        for ablation in self.ablations:
            ...
            for metric, values in ablation_metrics.items():
                ...
                if len(baseline_values) >= 2 and len(values) >= 2:
                    ...
                    cohens_d = (np.mean(baseline_values) - np.mean(values)) / (pooled_std + 1e-8)
    ...
    def _generate_markdown_report(self, analysis: Dict):
        ...
        for name, data in analysis["ablations"].items():
            ...
            lines.append(f"| {name} | {ssim_delta:+.4f} | {psnr_delta:+.2f} | {sig} |")
...
def run_ablation_suite(
    ...
    if include_interactions:
        study.add_interaction_study(
            ["loss.use_perceptual", "loss.use_contrastive", "model.use_attention"]
        )
