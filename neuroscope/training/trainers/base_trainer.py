...
class BaseTrainer(ABC):
    ...
    def _log_progress(self, batch_idx: int, num_batches: int, losses: dict[str, float]):
        ...
        print(
            f"epoch [{self.state.epoch + 1}/{self.config.num_epochs}] "
...
class GANTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        self.models = {"G": generator.to(self.device), "D": discriminator.to(self.device)}
...
class CycleGANTrainer(BaseTrainer):
    ...
    def build_optimizers(self) -> dict[str, torch.optim.Optimizer]:
        return {
            ...
            "D": torch.optim.Adam(
                list(self.models["D_A"].parameters()) + list(self.models["D_B"].parameters()),
                lr=self.config.lr_d,
                betas=(self.config.beta1, self.config.beta2),
            ),
        }
    ...
    def train_step(self, batch: dict[str, torch.Tensor]) -> dict[str, float]:
        ...
        with autocast(enabled=self.scaler is not None):
            ...
            loss_G = loss_gan_A2B + loss_gan_B2A + loss_cycle + loss_identity + loss_perceptual
        ...
        losses["loss_G"] = loss_G.item()
        losses["loss_cycle"] = (
            loss_cycle.item() if isinstance(loss_cycle, torch.Tensor) else loss_cycle
        )
        losses["loss_identity"] = (
            loss_identity.item() if isinstance(loss_identity, torch.Tensor) else loss_identity
        )
