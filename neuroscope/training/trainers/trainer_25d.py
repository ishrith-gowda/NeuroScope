...
from unified.data.dataset_25d import create_dataloaders
...
class Trainer:
    ...
    def __init__(
        ...
        self.scheduler_G = optim.lr_scheduler.CosineAnnealingLR(self.opt_G, T_max=100, eta_min=1e-6)
        self.scheduler_D = optim.lr_scheduler.CosineAnnealingLR(self.opt_D, T_max=100, eta_min=1e-6)
    ...
    def train(self, epochs: int, validate_every: int = 5, save_every: int = 10):
        ...
        for epoch in range(self.start_epoch, epochs):
            ...
            if (epoch + 1) % validate_every == 0:
                ...
                print(
                    f"  val ssim: a→b→a={val_metrics['ssim_A2B']:.4f}, b→a→b={val_metrics['ssim_B2A']:.4f}"
                )
                print(
                    f"  val psnr: a→b→a={val_metrics['psnr_A2B']:.2f}db, b→a→b={val_metrics['psnr_B2A']:.2f}db"
                )
