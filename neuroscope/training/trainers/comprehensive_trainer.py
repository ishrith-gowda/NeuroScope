...
class ComprehensiveTrainer:
    ...
    def setup_directories(self):
        ...
        for d in [self.checkpoints_dir, self.samples_dir, self.figures_dir, self.logs_dir]:
    ...
    def setup_logging(self):
        ...
        self.sample_generator = SampleGenerator(
            output_dir=self.samples_dir, modality_names=["T1", "T1Gd", "T2", "FLAIR"]
        )

        self.figure_generator = FigureGenerator(output_dir=self.figures_dir, style="publication")
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if torch.isnan(loss_G) or torch.isnan(loss_D):
                ...
                print(
                    f"   identity_a: {loss_identity_A.item()}, identity_b: {loss_identity_B.item()}"
                )
    ...
    def generate_samples(self):
        ...
        sample_paths = self.sample_generator.generate_all_samples(
            center_A, fake_B, rec_A, center_B, fake_A, rec_B, self.current_epoch
        )
    ...
    def train(self):
        ...
        self.logger.log_data_info(
            self.train_samples, self.val_samples, self.test_samples, self.config.batch_size
        )
        ...
        for epoch in range(self.current_epoch, self.config.epochs):
            ...
            self.history["identity_loss"].append(
            ...
            if self.current_epoch % self.config.validate_every == 0:
                ...
                if self.early_stopping:
                    state = CallbackState(
                        epoch=self.current_epoch, metrics={"val_ssim": avg_ssim}, model=self.model
                    )
                    self.early_stopping.on_epoch_end(state)

                    if self.early_stopping.stopped:
                        self.logger.log_early_stop(
                            self.current_epoch, self.config.patience, self.best_metric
                        )
                        break
            ...
            self.logger.on_epoch_end(
                self.current_epoch, train_metrics, val_metrics, new_lr, epoch_time
            )
        ...
        self.figure_generator.create_publication_summary(
            self.history, val_history, final_metrics, self.config.experiment_name
        )
