...
class ExponentialDecay(_LRScheduler):
    ...
    def __init__(
        self, optimizer: Optimizer, gamma: float = 0.99, min_lr: float = 0.0, last_epoch: int = -1
    ):
        """
        args:
            optimizer: wrapped optimizer
...
class CyclicScheduler(_LRScheduler):
    ...
    def get_lr(self):
        ...
        return [self.base_lr + (self.max_lr - self.base_lr) * phase * scale for _ in self.base_lrs]
...
class WarmRestarts(_LRScheduler):
    ...
    def get_lr(self):
        ...
        return [self.eta_min + factor * (base_lr - self.eta_min) for base_lr in self.base_lrs]
    ...
    **kwargs,
    ...
    if name == "linear":
        ...
        return ExponentialDecay(
            optimizer, gamma=kwargs.get("gamma", 0.99), min_lr=kwargs.get("min_lr", 0.0)
        )
