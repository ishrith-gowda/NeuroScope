...
class TensorBoardLogger:
    ...
    def _setup_custom_layout(self):
        ...
        layout = {
            "Training Losses": {
                "Generator": [
                    ...
                    [
                        ...
                        "Loss/Generator/gan",
                        "Loss/Generator/cycle",
                        "Loss/Generator/identity",
                    ],
                ],
                "Discriminator": [
                    "Multiline",
                    ["Loss/Discriminator/total", "Loss/Discriminator/A", "Loss/Discriminator/B"],
                ],
            },
            "Validation Metrics": {
                "SSIM": [
                    "Multiline",
                    ["Metrics/SSIM/A2B", "Metrics/SSIM/B2A", "Metrics/SSIM/mean"],
                ],
                "PSNR": [
                    "Multiline",
                    ["Metrics/PSNR/A2B", "Metrics/PSNR/B2A", "Metrics/PSNR/mean"],
                ],
            },
            "Learning Rates": {
            ...
            "Gradients": {
                "Norms": [
                    "Multiline",
                    ["Gradients/Generator/norm", "Gradients/Discriminator/norm"],
                ],
            },
        }
    ...
    def log_scalar(
        self, tag: str, value: float, step: Optional[int] = None, walltime: Optional[float] = None
    ):
    ...
    def log_validation_metrics(self, metrics: dict[str, float], step: Optional[int] = None):
        ...
        if "ssim_A2B" in metrics and "ssim_B2A" in metrics:
            self.log_scalar(
                "Metrics/SSIM/mean", (metrics["ssim_A2B"] + metrics["ssim_B2A"]) / 2, step
            )
        ...
        if "psnr_A2B" in metrics and "psnr_B2A" in metrics:
            self.log_scalar(
                "Metrics/PSNR/mean", (metrics["psnr_A2B"] + metrics["psnr_B2A"]) / 2, step
            )
    ...
    def log_learning_rates(self, lr_G: float, lr_D: float, step: Optional[int] = None):
        """log learning rates."""
        step = step if step is not None else self.step
    ...
    def log_model_weights(self, model: nn.Module, step: Optional[int] = None, prefix: str = ""):
        """
    ...
    def log_model_gradients(self, model: nn.Module, step: Optional[int] = None, prefix: str = ""):
        """
    ...
    def log_graph(self, model: nn.Module, input_shape: tuple, verbose: bool = False):
        """
