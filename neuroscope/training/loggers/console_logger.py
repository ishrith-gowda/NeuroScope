...
"""
    ...
    def print_model_summary(self, model_name: str, total_params: int, trainable_params: int):
        """print model summary."""
        if self.verbose < 1:
            return
    ...
    def on_epoch_start(self, epoch: int, total_epochs: int):
        ...
        if self.verbose >= 1:
            ...
            self._write(
                f"\n{self._color('epoch', Colors.BOLD)} {self._color(progress, Colors.CYAN)}"
            )
    ...
    def on_epoch_end(
        ...
        if self.verbose >= 1:
            ...
            self._write(f"  time: {self._format_time(epoch_time)} | eta: {self._format_time(eta)}")

        if self.verbose >= 2:
            # training metrics
            train_str = " | ".join([self._format_metric(k, v) for k, v in train_metrics.items()])
            self._write(f"  {self._color('train:', Colors.BLUE)} {train_str}")

            # validation metrics
            if val_metrics:
                val_str = " | ".join([self._format_metric(k, v) for k, v in val_metrics.items()])
