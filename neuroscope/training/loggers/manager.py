...
class LoggerManager:
    ...
    def __init__(
        ...
        if use_csv:
            self.csv_logger = CSVLogger(log_dir=self.metrics_dir, filename="training_metrics.csv")

        if use_json:
            self.json_logger = JSONLogger(log_dir=self.metrics_dir, filename="training_log.json")
    ...
    def log_model_info(
        ...
        if self.console_logger:
            self.console_logger.print_model_summary(model_name, total_params, trainable_params)
    ...
    def log_data_info(
        self, train_samples: int, val_samples: int, test_samples: int, batch_size: int
    ):
    ...
    def on_training_end(self, final_metrics: Optional[Dict[str, float]] = None):
        ...
        if self.json_logger:
            self.json_logger.log_event(
                "training_end", "Training completed", {"final_metrics": final_metrics}
            )
    ...
    def log_batch(
        ...
        if self.console_logger:
            self.console_logger.on_batch_end(batch_idx, total_batches, losses, samples_per_sec)
    ...
    def log_sample_comparison(
        ...
        if self.tb_logger:
            self.tb_logger.log_sample_comparison(
                real_A, fake_B, rec_A, real_B, fake_A, rec_B, step, modality_idx
            )
    ...
    def log_model_histograms(self, model: nn.Module, prefix: str = "", step: Optional[int] = None):
