...
class SampleGenerator:
    ...
    def __init__(
        ...
        for d in [self.comparisons_dir, self.modalities_dir, self.difference_dir, self.grids_dir]:
    ...
    def save_comparison(
        ...
        modality_name = (
            self.modality_names[modality_idx]
            if modality_idx < len(self.modality_names)
            else f"Mod{modality_idx}"
        )
        ...
        # add epoch info
        fig.suptitle(
            f"Epoch {epoch} - Batch {batch_idx} - {modality_name}", fontsize=12, fontweight="bold"
        )

        # save
        filename = (
            f"comparison_epoch{epoch:04d}_batch{batch_idx:04d}_{modality_name}.{self.save_format}"
        )
        filepath = self.comparisons_dir / filename
        plt.savefig(
            filepath, dpi=self.dpi, bbox_inches="tight", facecolor="white", edgecolor="none"
        )
        plt.close(fig)
    ...
    def save_multi_modality_comparison(
        ...
        for i in range(n_modalities):
            ...
            img_real_A = self._normalize_for_display(self._to_numpy(real_A[sample_idx, i]))
            img_fake_B = self._normalize_for_display(self._to_numpy(fake_B[sample_idx, i]))
            img_real_B = self._normalize_for_display(self._to_numpy(real_B[sample_idx, i]))
            img_fake_A = self._normalize_for_display(self._to_numpy(fake_A[sample_idx, i]))
        ...
        fig.suptitle(f"Multi-Modality Comparison - Epoch {epoch}", fontsize=14, fontweight="bold")
        plt.tight_layout()
    ...
    def generate_all_samples(
        ...
        for mod_idx in range(n_modalities):
            path = self.save_comparison(
                real_A, fake_B, rec_A, real_B, fake_A, rec_B, epoch, batch_idx, mod_idx
            )
            if path:
                results[f"comparison_{self.modality_names[mod_idx]}"] = path

        # multi-modality view
        path = self.save_multi_modality_comparison(real_A, fake_B, real_B, fake_A, epoch)
        if path:
        ...
        for mod_idx in range(min(2, n_modalities)):  # just first 2 modalities
            path = self.save_difference_map(
                real_A, rec_A, epoch, name="diff_A2B2A", modality_idx=mod_idx
            )
            if path:
