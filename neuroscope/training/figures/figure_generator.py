...
class FigureGenerator:
    ...
    def __init__(
        ...
        for d in [self.losses_dir, self.metrics_dir, self.analysis_dir, self.publication_dir]:
    ...
    def plot_training_losses(
        ...
        if "learning_rate" in history:
            ...
            axes[1, 1].plot(range(1, len(data) + 1), data, color="#34495e", linewidth=1.5)
    ...
    def generate_all_figures(
        ...
        if "G_loss" in history and "D_loss" in history:
            path = self.plot_generator_discriminator_balance(history["G_loss"], history["D_loss"])
            if path:
