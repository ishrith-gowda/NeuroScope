...
def create_cyclegan_optimizers(
    ...
    opt_g = torch.optim.Adam(g_params, lr=lr_g, betas=(beta1, beta2))

    opt_d = torch.optim.Adam(d_params, lr=lr_d, betas=(beta1, beta2))
    ...
    def create_optimizer(
        ...
        **kwargs,
        ...
        returns:
            configured optimizer
        """
        param_groups = DifferentialLR.create_param_groups(model, base_lr, layer_decay)
...
class ExponentialMovingAverage:
    ...
    def __init__(
        self, model: nn.Module, decay: float = 0.999, device: Optional[torch.device] = None
    ):
        """
        args:
