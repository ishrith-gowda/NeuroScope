    ...
    def on_epoch_end(self, state: CallbackState) -> None:
        ...
        if self._is_improvement(current):
            ...
            if self.restore_best_weights and state.model is not None:
                self.best_weights = {
                    k: v.cpu().clone() for k, v in state.model.state_dict().items()
                }
        else:
            self.counter += 1
...
class ModelCheckpoint(Callback):
    ...
    def _format_filepath(self, state: CallbackState) -> Path:
        name = self.filepath.name.format(epoch=state.epoch, step=state.step, **state.metrics)
...
class GradientMonitor(Callback):
    ...
    def __init__(self, model: nn.Module, log_freq: int = 100, log_histogram: bool = False):
        """
        args:
    ...
    def on_batch_end(self, state: CallbackState) -> None:
        if state.step % self.log_freq == 0:
            ...
            self.grad_norms.append(
                {"step": state.step, "total_norm": total_norm, "param_norms": param_norms}
            )
...
class GANMonitor(Callback):
    ...
    def __init__(
        self, log_freq: int = 100, track_d_g_ratio: bool = True, track_mode_collapse: bool = True
    ):
        """
        args:
            log_freq: steps between logs
