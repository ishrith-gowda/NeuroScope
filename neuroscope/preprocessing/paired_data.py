...
class MRIPairLoader:
    ...
    def __init__(
        ...
        if paired:
            ...
            if not common_filenames:
                logger.warning(
                    "No common filenames found between domains. Check directories or set paired=False."
                )
            else:
                # keep only matched files
                self.domain_a_files = [
                    f for f in self.domain_a_files if os.path.basename(f) in common_filenames
                ]
                self.domain_b_files = [
                    f for f in self.domain_b_files if os.path.basename(f) in common_filenames
                ]
    ...
    def load_domain_b(self, index: int) -> Tuple[np.ndarray, str]:
        ...
        if index >= len(self.domain_b_files):
            raise IndexError(
                f"Index {index} out of range for domain B with {len(self.domain_b_files)} items"
            )
...
def create_paired_dataset_splits(
    ...
    if paired:
        logger.info(
            f"Train: {len(train_files)}, Val: {len(val_files)}, Test: {len(test_files)} samples"
        )
    else:
        logger.info(
            f"Train: {len(train_files_dict['A'])} (A), {len(train_files_dict['B'])} (B) samples"
        )
        logger.info(f"Val: {len(val_files_dict['A'])} (A), {len(val_files_dict['B'])} (B) samples")
        logger.info(
            f"Test: {len(test_files_dict['A'])} (A), {len(test_files_dict['B'])} (B) samples"
        )
