...
class VolumeNormalization:
    ...
    def min_max_normalization(
        ...
        if v_min == v_max:
            logger.warning(
                f"Min and max values are equal: {v_min}. Setting normalized volume to {target_range[0]}."
            )
        ...
        try:
            ...
            for i in range(uint8_volume.shape[0]):  # xy planes
                clahe_xy[i] = exposure.equalize_adapthist(
                    uint8_volume[i], kernel_size=block_size[1:], clip_limit=clip_limit
                )
            ...
            for i in range(uint8_volume.shape[2]):  # yz planes
                clahe_yz[:, :, i] = exposure.equalize_adapthist(
                    uint8_volume[:, :, i], kernel_size=block_size[:2], clip_limit=clip_limit
                )
...
class DataAugmentation:
    ...
    def random_rotation_3d(
        ...
        for axis_pair in axes:
            ...
            result = rotate(result, angle, axes=axis_pair, reshape=False, mode=mode, order=order)
    ...
    def random_crop(
        ...
        for i in range(len(crop_size)):
            if crop_size[i] > volume.shape[i]:
                raise ValueError(
                    f"Crop size {crop_size[i]} is larger than volume size {volume.shape[i]} along axis {i}"
                )
        ...
        if mask is not None and np.any(mask > 0):
            ...
            starts = [
                max(0, min(valid_starts[i], center[i] - crop_size[i] // 2))
                for i in range(len(crop_size))
            ]
        else:
    ...
    def random_noise(
        ...
        if noise_type == "gaussian":
            ...
            noise = np.random.normal(loc=0, scale=std_dev * intensity_range, size=result.shape)
            ...
            if scaled > 0:
                ...
                noisy_scaled = np.random.poisson(lam=result[noise_mask] * lambda_factor / scaled)
            ...
            if step_name == "min_max_normalization":
                ...
                if "crop_size" in params:
                    ...
                    if "method" in params and params["method"] == "random":
                        ...
                        # center crop
                        starts = [
                            (result.shape[i] - crop_size[i]) // 2 for i in range(len(crop_size))
                        ]
                        slices = tuple(
                            slice(starts[i], starts[i] + crop_size[i])
                            for i in range(len(crop_size))
                        )
                ...
                if "scale_factor" in params:
                    ...
                    scale_factor = [
                        target_shape[i] / result.shape[i] for i in range(len(target_shape))
                    ]
                    order = params.get("order", 1)  # default to linear interpolation
                    result = zoom(result, scale_factor, order=order)
