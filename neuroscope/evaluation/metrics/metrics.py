...
class SSIM(nn.Module):
    ...
    def forward(
        ...
        sigma_x_sq = (
            F.conv2d(x**2, self.window, padding=self.window_size // 2, groups=channel) - mu_x_sq
        )
        sigma_y_sq = (
            F.conv2d(y**2, self.window, padding=self.window_size // 2, groups=channel) - mu_y_sq
        )
        sigma_xy = (
            F.conv2d(x * y, self.window, padding=self.window_size // 2, groups=channel) - mu_xy
        )
...
class LPIPS(nn.Module):
    ...
    def __init__(self, net: str = "vgg", pretrained: bool = True, spatial: bool = False):
        """
        args:
    ...
    def _build_vgg(self):
        ...
        self.weights = nn.ParameterList([nn.Parameter(torch.ones(1) * 0.1) for _ in range(5)])
...
class TumorPreservationScore(nn.Module):
    ...
    def __init__(self, threshold: float = 0.5, region_weights: Optional[Dict[str, float]] = None):
        ...
        self.region_weights = region_weights or {"enhancing": 1.0, "necrotic": 0.8, "edema": 0.6}
    ...
    def forward(
        ...
        # pearson correlation
        correlation = torch.corrcoef(torch.stack([orig_tumor.flatten(), harm_tumor.flatten()]))[
            0, 1
        ]
...
class VolumePreservation(nn.Module):
    ...
    def forward(self, original_seg: torch.Tensor, harmonized_seg: torch.Tensor) -> torch.Tensor:
        """
...
class MedicalImageMetrics:
    ...
    def compute_all(
        ...
        return {
            "tumor_preservation": self.tumor_preservation(original, harmonized, segmentation).item()
        }
