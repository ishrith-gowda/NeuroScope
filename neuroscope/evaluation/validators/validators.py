...
class BaseValidator(ABC):
    ...
    @abstractmethod
    def validate(
        self, train_fn: Callable, eval_fn: Callable, data_indices: np.ndarray
    ) -> ValidationSummary:
        """
        run validation.
...
class HoldoutValidator(BaseValidator):
    ...
    def validate(
        ...
        train_indices, val_indices, test_indices = self.get_train_val_test_split(data_indices)
...
class NestedCrossValidator:
    ...
    def __init__(self, outer_folds: int = 5, inner_folds: int = 3, random_state: int = 42):
        """
        args:
            ...
            random_state: random seed
        """
        self.outer_cv = CrossValidator(n_folds=outer_folds, random_state=random_state)
        self.inner_cv = CrossValidator(n_folds=inner_folds, random_state=random_state + 1)
    ...
    def validate(
        ...
        for fold, (train_indices, test_indices) in enumerate(outer_splits):
            ...
            best_hyperparams = hyperparams_fn(train_fn, eval_fn, train_indices, self.inner_cv)
