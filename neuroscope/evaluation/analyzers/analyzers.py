...
class ModalityAnalyzer:
    ...
    def add_result(
        ...
        if self.metrics_fn is not None:
            ...
            metrics = {
                ...
                "correlation": float(np.corrcoef(original.flatten(), harmonized.flatten())[0, 1]),
            }
    ...
    def get_summary(self) -> Dict[str, Dict[str, float]]:
        ...
        for modality, result in results.items():
            summary[modality] = {k: v for k, v in result.metrics.items() if k.endswith("_mean")}
    ...
    def add_result(
        ...
        for region, label in label_mapping.items():
            ...
            metrics = {
                ...
                "correlation": float(
                    np.corrcoef(orig_region.flatten(), harm_region.flatten())[0, 1]
                )
                if len(orig_region) > 1
                else 1.0,
...
class AblationAnalyzer:
    ...
    def add_configuration(
        ...
        if name not in self.configurations:
            self.configurations[name] = {"removed": removed_components, "metrics": []}
...
class CrossDatasetAnalyzer:
    ...
    def analyze(self) -> AnalysisResult:
        ...
        if len(self.target_datasets) >= 2:
            source_idx = (
                self.target_datasets.index(self.source_dataset)
                if self.source_dataset in self.target_datasets
                else 0
            )
    ...
    def get_generalization_score(self, metric_name: str = "ssim") -> float:
        """
