...
def anova_test(*groups: np.ndarray, alpha: float = 0.05) -> TestResult:
    """
...
def kruskal_wallis_test(*groups: np.ndarray, alpha: float = 0.05) -> TestResult:
    """
...
def compute_effect_size(x: np.ndarray, y: np.ndarray, method: str = "cohens_d") -> float:
    """
...
def interpret_effect_size(d: float, method: str = "cohens_d") -> str:
    """
...
def check_normality(data: np.ndarray, alpha: float = 0.05) -> TestResult:
    """
    ...
    return TestResult(statistic=statistic, p_value=p_value, test_name="Shapiro-Wilk", alpha=alpha)


def check_homogeneity(*groups: np.ndarray, alpha: float = 0.05) -> TestResult:
    """
    ...
    return TestResult(statistic=statistic, p_value=p_value, test_name="Levene's", alpha=alpha)
    ...
    def compare_methods(
        ...
        for method in methods:
            ...
            if norm_baseline.significant or norm_method.significant:
                ...
                if paired:
                    result = wilcoxon_test(baseline_data, method_data, self.alpha)
                else:
                    result = mann_whitney_test(baseline_data, method_data, self.alpha)
            else:
                # normal: use t-test
                if paired:
                    result = paired_t_test(baseline_data, method_data, self.alpha)
                else:
                    result = independent_t_test(baseline_data, method_data, self.alpha)
    ...
    def generate_report(self) -> dict:
        ...
        for name, result in self.results.items():
            report["tests"][name] = {
                ...
                "interpretation": interpret_effect_size(result.effect_size)
