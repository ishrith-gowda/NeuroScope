...
class TestResNetGenerator3D:
    ...
    def test_different_num_blocks(self, device):
        ...
        for num_blocks in [3, 6, 9]:
            gen = ResNetGenerator3D(input_nc=1, output_nc=1, num_residual_blocks=num_blocks).to(
                device
            )
            x = torch.randn(1, 1, 32, 32, 32, device=device)
...
class TestSAGenerator3D:
    ...
    def test_attention_positions(self, device):
        ...
        gen = SAGenerator3D(input_nc=1, output_nc=1, attention_positions=[2, 4, 6]).to(device)
        x = torch.randn(1, 1, 32, 32, 32, device=device)
...
class TestMemoryEfficientGenerator3D:
    ...
    def test_checkpointing(self, device):
        ...
        gen = MemoryEfficientGenerator3D(input_nc=1, output_nc=1, use_checkpoint=True).to(device)
...
class TestCycleGAN3D:
    ...
    def cyclegan(self, device):
        ...
        config = CycleGAN3DConfig(input_nc=1, output_nc=1, ngf=32, ndf=32, num_residual_blocks=4)
...
class TestGradientFlow:
    ...
    def test_full_cyclegan_gradients(self, device):
        ...
        config = CycleGAN3DConfig(input_nc=1, output_nc=1, ngf=16, ndf=16, num_residual_blocks=2)
...
class TestMemoryUsage:
    ...
    def test_checkpointing_reduces_memory(self, device):
        ...
        gen_no_ckpt = MemoryEfficientGenerator3D(input_nc=1, output_nc=1, use_checkpoint=False).to(
            device
        )
        ...
        # with checkpointing
        gen_ckpt = MemoryEfficientGenerator3D(input_nc=1, output_nc=1, use_checkpoint=True).to(
            device
        )
        x = torch.randn(1, 1, 48, 48, 48, device=device, requires_grad=True)
