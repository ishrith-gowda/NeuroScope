...
class TestOptimizers:
    ...
    def test_adam_optimizer_creation(self):
        ...
        optimizer = create_optimizer(
            model.parameters(), optimizer_type="adam", lr=0.0002, betas=(0.5, 0.999)
        )
    ...
    def test_adamw_optimizer(self):
        ...
        model = nn.Linear(10, 10)
        optimizer = create_optimizer(
            model.parameters(), optimizer_type="adamw", lr=0.0002, weight_decay=0.01
        )
...
class TestSchedulers:
    ...
    def test_warmup_cosine_scheduler(self):
        ...
        scheduler = create_scheduler(
            optimizer, scheduler_type="warmup_cosine", warmup_epochs=5, total_epochs=100
        )
...
class TestCallbacks:
    ...
    def test_model_checkpoint(self, tmp_path):
        ...
        callback = ModelCheckpoint(
            dirpath=str(tmp_path), filename="best", monitor="val_ssim", mode="max"
        )
...
class TestTrainer:
    ...
    def mock_trainer(self):
        ...
        trainer = HarmonizationTrainer(
            generator_a2b=G_A2B, generator_b2a=G_B2A, discriminator_a=D_A, discriminator_b=D_B
        )
    ...
    def test_train_step(self, mock_trainer):
        ...
        batch = {"source": torch.randn(1, 4, 16, 16, 16), "target": torch.randn(1, 4, 16, 16, 16)}
...
class TestGradientHandling:
    ...
    def test_gradient_clipping(self):
        ...
        grad_norm_after = sum(p.grad.norm() ** 2 for p in model.parameters()) ** 0.5
