...
class TestFullPipeline:
    ...
    def sample_data(self, temp_dir):
        ...
        for i in range(5):
            metadata["subjects"][f"subject_{i:03d}"] = {
                ...
                "files": {
                    "T1": str(domain_a_dir / f"volume_{i:03d}.npy")
                    if i < 3
                    else str(domain_b_dir / f"volume_{i:03d}.npy")
                },
            }
    ...
    def test_preprocessing_pipeline(self, temp_dir, sample_data):
        ...
        results = preprocessor.batch_process(
            input_dir=sample_data["domain_a_dir"], output_dir=output_dir, file_pattern="*.npy"
        )
    ...
    def test_training_setup(self):
        ...
        for loss_name, loss_value in epoch_losses.items():
            assert torch.isfinite(torch.tensor(loss_value)), (
                f"Loss {loss_name} is not finite: {loss_value}"
            )
    ...
    def test_loss_computation(self):
        ...
        d_losses = model.compute_discriminator_losses(
            real_a, real_b, outputs["fake_a"], outputs["fake_b"]
        )
