...
class SpectralNormDiscriminator(BaseDiscriminator):
    ...
    def __init__(self, in_channels: int = 4, ndf: int = 64, n_layers: int = 4):
        ...
        # first layer
        layers.append(spectral_norm(nn.Conv2d(in_channels, ndf, 4, stride=2, padding=1)))
        ...
        for i in range(1, n_layers):
            ...
            layers.append(spectral_norm(nn.Conv2d(in_ch, out_ch, 4, stride=2, padding=1)))
            layers.append(nn.LeakyReLU(0.2, inplace=True))

            in_ch = out_ch

        # output layer
        layers.append(spectral_norm(nn.Conv2d(in_ch, 1, 4, padding=1)))
...
class SNResNetDiscriminator(BaseDiscriminator):
    ...
    def __init__(self, in_channels: int = 4, ndf: int = 64, n_blocks: int = 4):
        ...
        # initial convolution
        self.initial = spectral_norm(nn.Conv2d(in_channels, ndf, 3, padding=1))
...
class SNProjectionDiscriminator(BaseDiscriminator):
    ...
    def __init__(self, in_channels: int = 4, ndf: int = 64, n_classes: int = 0):
        super().__init__(in_channels, ndf)
...
class SNSelfAttentionDiscriminator(BaseDiscriminator):
    ...
    def __init__(
        self, in_channels: int = 4, ndf: int = 64, n_layers: int = 5, attention_layer: int = 2
    ):
        super().__init__(in_channels, ndf)
        ...
        for i in range(n_layers):
            ...
            self.layers.append(SNConvBlock(in_ch, out_ch, stride=2))
