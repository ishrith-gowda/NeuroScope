        ...
        for i in range(n_scales):
            ...
            self.discriminators.append(
                NLayerPatchDiscriminator(
                    in_channels=in_channels, ndf=scale_ndf, n_layers=n_layers, norm_type=norm_type
                )
            )
...
class PyramidDiscriminator(BaseDiscriminator):
    ...
    def __init__(self, in_channels: int = 4, ndf: int = 64, n_levels: int = 4):
        ...
        for i in range(n_levels):
            ...
            # lateral connection
            self.lateral_convs.append(nn.Conv2d(out_ch, ndf * 2, 1))
        ...
        for i in range(self.n_levels - 1, -1, -1):
            ...
            if prev_features is not None:
                ...
                upsampled = F.interpolate(
                    prev_features, size=lateral.shape[-2:], mode="bilinear", align_corners=False
                )
...
class SharedEncoderMultiScaleDiscriminator(BaseDiscriminator):
    ...
    def __init__(self, in_channels: int = 4, ndf: int = 64, n_scales: int = 3):
        super().__init__(in_channels, ndf)
        ...
        channels_per_scale = [min(ndf * (2**i), 512) for i in range(5)]
...
class ProgressiveMultiScaleDiscriminator(BaseDiscriminator):
    ...
    def __init__(self, in_channels: int = 4, ndf: int = 64, max_scales: int = 4):
        super().__init__(in_channels, ndf)
        ...
        for i in range(max_scales):
            ...
            self.discriminators.append(
                NLayerPatchDiscriminator(
                    in_channels=in_channels, ndf=ndf, n_layers=n_layers, norm_type="instance"
                )
            )
...
class DualScaleDiscriminator(BaseDiscriminator):
    ...
    def __init__(
        self, in_channels: int = 4, ndf: int = 64, global_layers: int = 5, local_layers: int = 3
    ):
        super().__init__(in_channels, ndf)

        # global discriminator (full image)
        self.global_disc = NLayerPatchDiscriminator(
            in_channels=in_channels, ndf=ndf, n_layers=global_layers, norm_type="instance"
        )
        ...
        if patch_size < H and patch_size < W:
            ...
            for i in range(B):
                patch = x[
                    i : i + 1,
                    :,
                    h_start[i] : h_start[i] + patch_size,
                    w_start[i] : w_start[i] + patch_size,
                ]
