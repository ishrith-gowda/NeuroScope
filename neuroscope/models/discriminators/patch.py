...
class NLayerPatchDiscriminator(PatchDiscriminator):
    ...
    def _build_network(self) -> nn.Sequential:
        ...
        layers.append(nn.Conv2d(self._in_channels, self._ndf, 4, stride=2, padding=1))
        ...
        for n in range(1, self._n_layers):
            ...
            layers.append(
                nn.Conv2d(self._ndf * nf_mult_prev, self._ndf * nf_mult, 4, stride=2, padding=1)
            )
        ...
        layers.append(
            nn.Conv2d(self._ndf * nf_mult_prev, self._ndf * nf_mult, 4, stride=1, padding=1)
        )
        if norm_layer is not None:
            layers.append(norm_layer(self._ndf * nf_mult))
        layers.append(nn.LeakyReLU(0.2, inplace=True))
        ...
        # output layer
        layers.append(nn.Conv2d(self._ndf * nf_mult, 1, 4, stride=1, padding=1))
...
class PixelDiscriminator(BaseDiscriminator):
    ...
    def __init__(self, in_channels: int = 4, ndf: int = 64, norm_type: str = "instance"):
...
class DeepPatchDiscriminator(BaseDiscriminator):
    ...
    def __init__(self, in_channels: int = 4, ndf: int = 64, n_layers: int = 5):
        super().__init__(in_channels, ndf)
...
class ResidualPatchDiscriminator(BaseDiscriminator):
    ...
    def __init__(self, in_channels: int = 4, ndf: int = 64, n_blocks: int = 4):
        super().__init__(in_channels, ndf)
...
class ResidualDownBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int):
        ...
        self.shortcut = nn.Sequential(nn.AvgPool2d(2), nn.Conv2d(in_channels, out_channels, 1))
...
class DilatedPatchDiscriminator(BaseDiscriminator):
    ...
    def __init__(self, in_channels: int = 4, ndf: int = 64, n_layers: int = 4):
        super().__init__(in_channels, ndf)

        layers = []

        # first layer (no dilation)
        layers.append(nn.Conv2d(in_channels, ndf, 3, stride=1, padding=1))
        layers.append(nn.LeakyReLU(0.2, inplace=True))
        ...
        for i in range(n_layers):
            ...
            layers.append(nn.Conv2d(in_ch, out_ch, 3, stride=1, padding=padding, dilation=dilation))
...
class AttentionPatchDiscriminator(BaseDiscriminator):
    ...
    def __init__(
        self, in_channels: int = 4, ndf: int = 64, n_layers: int = 4, attention_layer: int = 2
    ):
        super().__init__(in_channels, ndf)
