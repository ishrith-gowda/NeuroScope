...
class IdentityLoss(nn.Module):
    ...
    def forward(self, real: torch.Tensor, same_domain_output: torch.Tensor) -> torch.Tensor:
...
class SSIMLoss(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        sigma_x_sq = (
            F.conv2d(x * x, window, padding=self.window_size // 2, groups=channels) - mu_x_sq
        )
        sigma_y_sq = (
            F.conv2d(y * y, window, padding=self.window_size // 2, groups=channels) - mu_y_sq
        )
