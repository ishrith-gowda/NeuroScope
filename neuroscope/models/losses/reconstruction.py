...
class SSIMLoss(nn.Module):
    ...
    def __init__(
        self, window_size: int = 11, sigma: float = 1.5, size_average: bool = True, channel: int = 1
    ):
        super().__init__()
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        sigma1_sq = (
            F.conv2d(pred**2, window, padding=self.window_size // 2, groups=channel) - mu1_sq
        )
        sigma2_sq = (
            F.conv2d(target**2, window, padding=self.window_size // 2, groups=channel) - mu2_sq
        )
        sigma12 = (
            F.conv2d(pred * target, window, padding=self.window_size // 2, groups=channel) - mu1_mu2
        )
...
class FocalFrequencyLoss(nn.Module):
    ...
    def forward(
        self, pred: torch.Tensor, target: torch.Tensor, weight_matrix: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
