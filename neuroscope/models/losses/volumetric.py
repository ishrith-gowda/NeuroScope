...
class VolumetricSSIM(nn.Module):
    ...
    def forward(self, x: Tensor, y: Tensor) -> Tensor:
        ...
        # compute variances
        sigma_x_sq = (
            F.conv3d(x * x, window, padding=self.window_size // 2, groups=channel) - mu_x_sq
        )
        sigma_y_sq = (
            F.conv3d(y * y, window, padding=self.window_size // 2, groups=channel) - mu_y_sq
        )
...
class VolumetricMultiScaleSSIM(nn.Module):
    ...
    def __init__(
        ...
        self.ssim = VolumetricSSIM(
            window_size=window_size, channel=channel, size_average=False, data_range=data_range
        )
...
class VolumetricCycleConsistencyLoss(nn.Module):
    ...
    def forward(self, real: Tensor, reconstructed: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        """
    ...
    def _single_scale_loss(
        self, real: Tensor, reconstructed: Tensor, mask: Optional[Tensor]
    ) -> Tensor:
...
class VolumetricGradientLoss(nn.Module):
    ...
    def __init__(self, loss_type: str = "l1", edge_weight: float = 1.0):
        """
    ...
    def _compute_gradients(self, x: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        grad_x = F.conv3d(x.view(B * C, 1, D, H, W), self.sobel_x.to(x.device), padding=1).view(
            B, C, D, H, W
        )

        grad_y = F.conv3d(x.view(B * C, 1, D, H, W), self.sobel_y.to(x.device), padding=1).view(
            B, C, D, H, W
        )

        grad_z = F.conv3d(x.view(B * C, 1, D, H, W), self.sobel_z.to(x.device), padding=1).view(
            B, C, D, H, W
        )
...
class AnatomicalConsistencyLoss(nn.Module):
    ...
    def _estimate_tissue_priors(self, volume: Tensor) -> Dict[str, Tensor]:
        ...
        return {"csf": csf_mask, "gm": gm_mask, "wm": wm_mask}
...
class TissuePreservationLoss(nn.Module):
    ...
    def _compute_boundaries(self, x: Tensor) -> Tensor:
        ...
        boundaries = F.conv3d(x.view(B * C, 1, D, H, W), self.laplacian.to(x.device), padding=1)
...
class VolumetricIdentityLoss(nn.Module):
    ...
    def __init__(self, loss_type: str = "l1", lambda_ssim: float = 0.0):
        """
...
class CombinedVolumetricLoss(nn.Module):
    ...
    def __init__(
        ...
        self.cycle_loss = VolumetricCycleConsistencyLoss(loss_type="combined", multi_scale=True)
