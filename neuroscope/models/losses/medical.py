...
class TumorPreservationLoss(nn.Module):
    ...
    def forward(
        ...
        intensity_loss = F.l1_loss(output_img * tumor_mask, input_img * tumor_mask)

        # boundary preservation
        input_edges = self._compute_edges(input_img)
        output_edges = self._compute_edges(output_img)
        boundary_loss = F.l1_loss(output_edges * tumor_mask, input_edges * tumor_mask)
...
class RadiomicsPreservationLoss(nn.Module):
    ...
    def __init__(
        self, texture_weight: float = 1.0, shape_weight: float = 0.5, intensity_weight: float = 1.0
    ):
        super().__init__()
...
class AnatomicalConsistencyLoss(nn.Module):
    ...
    def _gradient_structure_loss(
        self, input_img: torch.Tensor, output_img: torch.Tensor, mask: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
...
class ContrastEnhancementLoss(nn.Module):
    ...
    def forward(
        ...
        enhancement_mask = (
            input_enhancement > self.enhancement_threshold * input_enhancement.max()
        ).float()

        # preserve enhancement in detected regions
        loss = F.l1_loss(
            output_enhancement * enhancement_mask, input_enhancement * enhancement_mask
        )
...
class NormalizedCrossCorrelationLoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        if ndims == 2:
            ...
            sum_filt = torch.ones(
                1, 1, self.window_size, self.window_size, self.window_size, device=pred.device
            )
        ...
        cross = (
            pred_target_sum
            - target_mean * pred_sum
            - pred_mean * target_sum
            + pred_mean * target_mean * win_size
        )
