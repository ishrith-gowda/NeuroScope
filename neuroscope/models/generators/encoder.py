    ...
    def __init__(
        ...
        for i in range(n_downsample):
            ...
            self.downsample_layers.append(
                DownsampleBlock(
                    current_channels, out_channels, norm_type=norm_type, activation=activation
                )
            )
...
class MultiModalEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.modality_encoders = nn.ModuleList(
            [
                ConvEncoder(in_channels=1, base_channels=base_channels, n_downsample=n_downsample)
        ...
        if fusion_type == "concat":
            self.fusion = nn.Conv2d(encoder_out_channels * n_modalities, encoder_out_channels, 1)
            self.output_channels = encoder_out_channels

        elif fusion_type == "attention":
            self.fusion = ModalityAttentionFusion(n_modalities, encoder_out_channels)
            self.output_channels = encoder_out_channels
...
class HierarchicalEncoder(nn.Module):
    ...
    def __init__(self, in_channels: int, base_channels: int = 64, n_scales: int = 4):
        super().__init__()
