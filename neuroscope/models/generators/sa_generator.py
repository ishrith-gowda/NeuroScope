...
class SAGenerator(BaseGenerator):
    ...
    def __init__(
        ...
        self.encoder = SAEncoder(ngf, n_downsampling, norm_layer, attention_type, use_spectral_norm)
        ...
        self.bottleneck = SABottleneck(
            bottleneck_channels, n_residual, norm_layer, attention_type, use_spectral_norm
        )

        # decoder with attention
        self.decoder = SADecoder(
            bottleneck_channels, ngf, n_downsampling, norm_layer, attention_type, use_spectral_norm
        )
...
class SABottleneck(nn.Module):
    ...
    def __init__(
        ...
        for i in range(n_residual):
            # residual block
            self.blocks.append(ResidualBlock(channels, norm_type="instance"))
...
class MultiScaleSAGenerator(BaseGenerator):
    ...
    def __init__(
        self, in_channels: int = 4, out_channels: int = 4, ngf: int = 64, n_scales: int = 3
    ):
        super().__init__(in_channels, out_channels, ngf)
        ...
        for i in range(n_scales):
            ...
            self.scale_generators.append(
                SAGenerator(in_channels, out_channels, scale_ngf, n_residual=6, n_downsampling=2)
            )
