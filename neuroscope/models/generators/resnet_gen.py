...
class ResNetGenerator(BaseGenerator):
    ...
    def __init__(
        ...
        self.final = nn.Sequential(
            padding_layer(3), nn.Conv2d(ngf, out_channels, kernel_size=7, padding=0), nn.Tanh()
        )
...
class FastResNetGenerator(BaseGenerator):
    ...
    def __init__(
        self, in_channels: int = 4, out_channels: int = 4, ngf: int = 32, n_residual: int = 6
    ):
        super().__init__(in_channels, out_channels, ngf)
        ...
        self.residual_blocks = nn.Sequential(
            *[LightweightResidualBlock(ngf * 2) for _ in range(n_residual)]
        )
        ...
        # final convolution
        self.final = nn.Sequential(
            nn.ReflectionPad2d(2), nn.Conv2d(ngf, out_channels, kernel_size=5, padding=0), nn.Tanh()
        )
    ...
    def __init__(
        ...
        for i in range(n_downsampling):
            ...
            self.upsample.append(
                nn.Sequential(
                    nn.ConvTranspose2d(
                        ngf * mult, ngf * out_mult, 3, stride=2, padding=1, output_padding=1
                    ),
                    nn.InstanceNorm2d(ngf * out_mult),
                    nn.ReLU(inplace=True),
                )
        ...
        # final convolution
        self.final = nn.Sequential(
            nn.ReflectionPad2d(3), nn.Conv2d(ngf, out_channels, kernel_size=7, padding=0), nn.Tanh()
        )
