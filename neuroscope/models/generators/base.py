...
class BaseGenerator(nn.Module, ABC):
    ...
    def __init__(self, in_channels: int, out_channels: int, ngf: int = 64, **kwargs):
        """
...
class EncoderDecoderGenerator(BaseGenerator):
    ...
    def decode(self, x: torch.Tensor, skip_features: Optional[list] = None) -> torch.Tensor:
        """
...
class MultiScaleGenerator(BaseGenerator):
    ...
    def __init__(
        self, in_channels: int, out_channels: int, ngf: int = 64, num_scales: int = 3, **kwargs
    ):
        super().__init__(in_channels, out_channels, ngf, **kwargs)
    ...
    def forward(self, x: torch.Tensor, return_all_scales: bool = False) -> torch.Tensor:
        """
        forward pass.
...
class ConditionalGenerator(BaseGenerator):
    ...
    def __init__(
        self, in_channels: int, out_channels: int, ngf: int = 64, condition_dim: int = 128, **kwargs
    ):
        super().__init__(in_channels, out_channels, ngf, **kwargs)
    ...
    @abstractmethod
    def generate(
        self, x: torch.Tensor, condition_embedding: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
...
class StyleGenerator(BaseGenerator):
    ...
    @abstractmethod
    def synthesize(self, x: torch.Tensor, w: Optional[torch.Tensor] = None) -> torch.Tensor:
