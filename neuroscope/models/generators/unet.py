...
class UNetGenerator(BaseGenerator):
    ...
    def __init__(
        ...
        for i in range(n_downsampling - 1, -1, -1):
            ...
            self.decoder.append(
                UNetDecoderBlock(
                    in_ch + out_ch, out_ch, norm_layer, use_dropout if i > 0 else False
                )
            )
            in_ch = out_ch

        # final layer
        self.final = nn.Sequential(nn.ConvTranspose2d(in_ch, out_channels, 4, 2, 1), nn.Tanh())
...
class UNetEncoderBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, norm_layer, use_dropout: bool = False):
        super().__init__()
...
class AttentionUNetGenerator(BaseGenerator):
    ...
    def __init__(
        self, in_channels: int = 4, out_channels: int = 4, ngf: int = 64, n_downsampling: int = 4
    ):
        super().__init__(in_channels, out_channels, ngf)
        ...
        for i in range(n_downsampling - 1, -1, -1):
            ...
            # attention gate
            self.attention_gates.append(AttentionGate(out_ch, in_ch, out_ch // 2))

            # upsample
            self.upsample.append(nn.ConvTranspose2d(in_ch, out_ch, 2, 2))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, (attn_gate, up, dec_block) in enumerate(
            ...
            if out.shape[-2:] != attended_skip.shape[-2:]:
                out = F.interpolate(
                    out, size=attended_skip.shape[-2:], mode="bilinear", align_corners=False
                )
...
class AttentionGate(nn.Module):
    ...
    def __init__(self, skip_channels: int, gating_channels: int, inter_channels: int):
        ...
        self.W_skip = nn.Sequential(
            nn.Conv2d(skip_channels, inter_channels, 1), nn.BatchNorm2d(inter_channels)
        )

        self.W_gate = nn.Sequential(
            nn.Conv2d(gating_channels, inter_channels, 1), nn.BatchNorm2d(inter_channels)
        )
    ...
    def forward(self, skip: torch.Tensor, gating: torch.Tensor) -> torch.Tensor:
        ...
        gating_resized = F.interpolate(
            gating, size=skip.shape[-2:], mode="bilinear", align_corners=False
        )
...
class ResUNetGenerator(BaseGenerator):
    ...
    def __init__(
        ...
        self.initial = nn.Sequential(
            nn.Conv2d(in_channels, ngf, 3, padding=1), nn.BatchNorm2d(ngf), nn.ReLU(inplace=True)
        )
        ...
        # bottleneck
        self.bottleneck = nn.Sequential(ResidualBlock(in_ch), ResidualBlock(in_ch))
        ...
        # final
        self.final = nn.Sequential(nn.Conv2d(in_ch, out_channels, 1), nn.Tanh())
...
class UNetPlusPlusGenerator(BaseGenerator):
    ...
    def __init__(self, in_channels: int = 4, out_channels: int = 4, ngf: int = 64, depth: int = 4):
        super().__init__(in_channels, out_channels, ngf)
        ...
        for i in range(depth + 1):
            for j in range(depth + 1 - i):
                ...
                self.conv[f"x_{i}_{j}"] = ConvBlock(
                    in_ch if i > 0 or j > 0 else in_channels, out_ch
                )
        ...
        for i in range(1, self.depth + 1):
            for j in range(self.depth + 1 - i):
                ...
                if up_output.shape[-2:] != prev_outputs[0].shape[-2:]:
                    up_output = F.interpolate(
                        up_output,
                        size=prev_outputs[0].shape[-2:],
                        mode="bilinear",
                        align_corners=False,
                    )
