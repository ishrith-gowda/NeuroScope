...
class ConvDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.final = nn.Sequential(
            nn.ReflectionPad2d(3), nn.Conv2d(current_channels, out_channels, 7), nn.Tanh()
        )
        ...
        for i in range(n_upsample):
            ...
            stage = nn.ModuleList(
                [
                    ...
                    # residual blocks
                    nn.Sequential(
                        *[ResidualBlock(out_ch, norm_type=norm_type) for _ in range(n_residual)]
                    ),
                ]
            )
...
class UNetDecoder(nn.Module):
    ...
    def __init__(
        ...
        for i in range(n_stages):
            ...
            self.up_convs.append(nn.ConvTranspose2d(current_channels, out_ch, 2, stride=2))
...
class AttentionDecoder(nn.Module):
    ...
    def __init__(
        ...
        for i in range(n_stages):
            ...
            # attention gate
            self.attention_gates.append(AttentionGate(skip_ch, current_channels, skip_ch // 2))
...
class ProgressiveDecoder(nn.Module):
    ...
    def __init__(
        self, in_channels: int, out_channels: int, base_channels: int = 64, n_scales: int = 3
    ):
        super().__init__()
        ...
        for i in range(n_scales):
            ...
            self.to_rgb.append(nn.Conv2d(out_ch, out_channels, 1))
...
class PixelShuffleDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.final = nn.Sequential(
            nn.Conv2d(current_channels, out_channels, 3, padding=1), nn.Tanh()
        )
