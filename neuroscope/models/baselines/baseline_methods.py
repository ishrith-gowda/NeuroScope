...
class ComBat:
    ...
    def fit(self, data: np.ndarray, batch: np.ndarray, covariates: Optional[np.ndarray] = None):
        """
    ...
    def _parametric_eb(
        ...
        for i in range(n_batch):
            ...
            for j in range(n_features):
                ...
                # delta posterior (ig)
                delta_star[i, j] = (theta_bar + 0.5 * (n_b - 1) * delta_hat[i, j]) / (
    ...
    def transform(self, data: np.ndarray, batch: np.ndarray, target_batch: int = 0) -> np.ndarray:
        """
        ...
        for i, b in enumerate(self.batches):
            ...
            if np.any(mask):
                ...
                # remove batch effect
                harmonized[mask] = (batch_data - self.gamma_star[i]) / np.sqrt(
        ...
        harmonized = (
            ...
            + self.grand_mean
            + self.gamma_star[target_idx] * np.sqrt(self.var_pooled)
        )
...
class ComBatTorch(nn.Module):
    ...
    def forward(self, x: torch.Tensor, batch: torch.Tensor, target_batch: int = 0) -> torch.Tensor:
        """
        ...
        for i in range(self.n_batches):
            mask = batch == i
            if mask.any():
                x_corrected[mask] = (x_stand[mask] - self.gamma[i]) / torch.sqrt(delta[i] + 1e-8)

        # transform to target batch
        x_out = (
            x_corrected * torch.sqrt(self.running_var + 1e-8) * torch.sqrt(delta[target_batch])
            + self.running_mean
            + self.gamma[target_batch] * torch.sqrt(self.running_var + 1e-8)
        )
...
class PatchSampleF(nn.Module):
    ...
    def __init__(self, nc: int = 256, use_mlp: bool = True, init_type: str = "normal"):
        super().__init__()
        self.nc = nc
        self.use_mlp = use_mlp

        if use_mlp:
            self.mlp = nn.Sequential(nn.Linear(nc, nc), nn.ReLU(inplace=True), nn.Linear(nc, nc))
...
class PatchNCELoss(nn.Module):
    ...
    def __init__(self, nce_t: float = 0.07, nce_includes_all_negatives: bool = True):
        super().__init__()
        self.nce_t = nce_t
    ...
    def forward(
        self, feat_q: torch.Tensor, feat_k: torch.Tensor, feat_k_neg: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
        ...
        # positive logits
        l_pos = torch.bmm(feat_q.view(B * N, 1, C), feat_k.view(B * N, C, 1)).squeeze(
            -1
        )  # [b*n, 1]
        ...
        l_neg = torch.bmm(
            feat_q.view(B * N, 1, C), feat_k_neg.view(B, N, C).permute(0, 2, 1)
        )  # [b*n, n]
...
class CUTGenerator(nn.Module):
    ...
    def __init__(
        ...
        for i in range(n_downsampling):
            mult = 2**i
            model.append(nn.Conv2d(ngf * mult, ngf * mult * 2, 3, stride=2, padding=1, bias=False))
    ...
    def forward(self, x: torch.Tensor, encode_only: bool = False) -> torch.Tensor:
        """
        forward pass.
...
class VAEEncoder(nn.Module):
    ...
    def __init__(
        self, input_nc: int = 4, ngf: int = 64, n_downsampling: int = 3, latent_dim: int = 256
    ):
        super().__init__()
...
class VAEDecoder(nn.Module):
    ...
    def __init__(
        self, output_nc: int = 4, ngf: int = 64, n_upsampling: int = 3, latent_dim: int = 256
    ):
        super().__init__()
        ...
        for i in range(n_upsampling):
            ...
            layers.extend(
                [
                    nn.ConvTranspose2d(
                        ngf * mult, ngf * mult // 2, 4, stride=2, padding=1, bias=False
                    ),
                    nn.InstanceNorm2d(ngf * mult // 2),
                    nn.ReLU(inplace=True),
                ]
        ...
        layers.extend([nn.ReflectionPad2d(3), nn.Conv2d(ngf, output_nc, 7), nn.Tanh()])
...
class UNIT(nn.Module):
    ...
    def __init__(self, input_nc: int = 4, output_nc: int = 4, ngf: int = 64, latent_dim: int = 256):
        super().__init__()
...
class HistogramMatching:
    ...
    def transform(self, source: np.ndarray) -> np.ndarray:
        ...
        # apply mapping
        source_indices = np.clip(np.digitize(source, src_edges) - 1, 0, self.n_bins - 1)
