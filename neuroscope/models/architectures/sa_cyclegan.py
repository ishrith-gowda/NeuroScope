...
class SAGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.output = nn.Sequential(
            nn.ReflectionPad2d(3), nn.Conv2d(ngf, output_channels, 7), nn.Tanh()
        )
...
class SADiscriminator(nn.Module):
    ...
    def __init__(
        ...
        for n in range(1, n_layers):
            ...
            sequence.extend(
                [
                    get_norm(
                        nn.Conv2d(
                            ndf * nf_mult_prev, ndf * nf_mult, 4, stride=2, padding=1, bias=False
                        )
                    ),
        ...
        sequence.append(get_norm(nn.Conv2d(ndf * nf_mult, 1, 4, stride=1, padding=1)))
...
class MultiScaleDiscriminator(nn.Module):
    ...
    def __init__(
        self, input_channels: int = 4, ndf: int = 64, n_layers: int = 3, n_scales: int = 2
    ):
        super().__init__()

        self.n_scales = n_scales
        self.discriminators = nn.ModuleList(
            [SADiscriminator(input_channels, ndf, n_layers) for _ in range(n_scales)]
        )
...
def create_sa_cyclegan(
    input_channels: int = 4, ngf: int = 64, ndf: int = 64, n_residual_blocks: int = 9
) -> SACycleGAN:
    """
    ...
    """
    model = SACycleGAN(
        input_channels=input_channels, ngf=ngf, ndf=ndf, n_residual_blocks=n_residual_blocks
    )
