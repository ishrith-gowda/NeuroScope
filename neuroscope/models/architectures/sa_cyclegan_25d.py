...
class SAGenerator25D(nn.Module):
    ...
    def __init__(self, config: SACycleGAN25DConfig):
        ...
        self.encoder_initial = SliceEncoder25D(
            n_slices=config.n_input_slices, n_modalities=config.n_modalities, ngf=ngf
        )
        ...
        for i in range(config.n_residual_blocks):
            ...
            self.bottleneck.append(
                ResidualBlock(
                    ngf * 4, use_attention=True, attention_type="self" if use_self_attn else "cbam"
                )
            )
        ...
        self.skip_fuse = nn.ModuleList(
            [nn.Conv2d(ngf * 4, ngf * 2, 1, bias=False), nn.Conv2d(ngf * 2, ngf, 1, bias=False)]
        )

        # output: center slice only (4 modalities)
        self.output = nn.Sequential(
            nn.ReflectionPad2d(3), nn.Conv2d(ngf, config.output_channels, 7), nn.Tanh()
        )
