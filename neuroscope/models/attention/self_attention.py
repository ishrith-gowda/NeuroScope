...
class SelfAttention2d(nn.Module):
    ...
    def __init__(self, in_channels: int, reduction: int = 8, use_spectral_norm: bool = True):
        super().__init__()
    ...
    def forward(
        ...
        attention = self.softmax(
            torch.bmm(query, key) / math.sqrt(self.reduced_channels)
        )  # [b, hw, hw]
...
class MultiScaleSelfAttention(nn.Module):
    ...
    def __init__(self, in_channels: int, scales: tuple[int, ...] = (1, 2, 4), reduction: int = 8):
        ...
        self.attention_layers = nn.ModuleList(
            [SelfAttention2d(in_channels, reduction, use_spectral_norm=True) for _ in scales]
        )
