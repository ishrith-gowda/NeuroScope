...
class VGG16Features(nn.Module):
    ...
    def __init__(
        ...
        # store layer indices
        self.layer_indices = {
            layer: name_to_idx[layer] for layer in self.feature_layers if layer in name_to_idx
        }
...
class VGGPerceptualExtractor(nn.Module):
    ...
    def compute_loss(
        self, pred: torch.Tensor, target: torch.Tensor, loss_type: str = "l1"
    ) -> torch.Tensor:
        """
...
class MultiLayerVGG(nn.Module):
    ...
    def __init__(
        ...
        self.style_layers = style_layers or ["relu1_2", "relu2_2", "relu3_4", "relu4_4", "relu5_4"]

        all_layers = list(set(self.content_layers + self.style_layers))

        self.backbone = VGG19Features(
            feature_layers=all_layers, pretrained=pretrained, normalize_input=normalize_input
        )
    ...
    def forward(self, x: torch.Tensor) -> tuple[dict[str, torch.Tensor], dict[str, torch.Tensor]]:
        ...
        content_features = {
            layer: all_features[layer] for layer in self.content_layers if layer in all_features
        }
    ...
    def compute_content_loss(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        for layer in self.content_layers:
            if layer in pred_content and layer in target_content:
                loss = loss + torch.pow(pred_content[layer] - target_content[layer], 2).mean()

        return loss

    def compute_style_loss(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        for layer in self.style_layers:
            if layer in pred_style and layer in target_style:
                loss = loss + torch.pow(pred_style[layer] - target_style[layer], 2).mean()
