...
class CycleGAN3D(nn.Module):
    ...
    def compute_generator_loss(
        ...
        if isinstance(pred_fake_B, list):
            loss_gan_A2B = sum(
                self.criterion_gan(p, torch.ones_like(p)) for p in pred_fake_B
            ) / len(pred_fake_B)
            loss_gan_B2A = sum(
                self.criterion_gan(p, torch.ones_like(p)) for p in pred_fake_A
            ) / len(pred_fake_A)
        else:
...
class SACycleGAN3D(CycleGAN3D):
    ...
    def compute_generator_loss(
        ...
        # base losses
        total_loss, loss_dict = super().compute_generator_loss.__wrapped__(self, real_A, real_B)
        ...
        if tumor_mask_A is not None and hasattr(self, "tumor_loss"):
            loss_tumor_A = self.tumor_loss(outputs["fake_B"], real_A, tumor_mask_A)
        ...
        if tumor_mask_B is not None and hasattr(self, "tumor_loss"):
            loss_tumor_B = self.tumor_loss(outputs["fake_A"], real_B, tumor_mask_B)
...
class TumorPreservation3DLoss(nn.Module):
    ...
    def __init__(self, intensity_weight: float = 1.0, gradient_weight: float = 0.5):
        super().__init__()
    ...
    def forward(
        self, generated: torch.Tensor, original: torch.Tensor, mask: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        return self.intensity_weight * intensity_loss + self.gradient_weight * gradient_loss
