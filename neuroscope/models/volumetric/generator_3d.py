...
class SAGenerator3D(nn.Module):
    ...
    def __init__(
        ...
        # final convolution
        self.final = nn.Sequential(
            nn.ReplicationPad3d(3), nn.Conv3d(ngf, out_channels, 7, 1, 0), nn.Tanh()
        )

    def _build_attention(
        self, channels: int, attention_type: str, use_checkpoint: bool
    ) -> nn.Module:
        """build attention module based on type."""
        if attention_type == "self":
            return SelfAttention3D(channels, use_checkpoint=use_checkpoint)
        elif attention_type == "multi_head":
            return MultiHeadSelfAttention3D(channels, num_heads=8, use_checkpoint=use_checkpoint)
...
class UNetGenerator3D(nn.Module):
    ...
    def __init__(
        ...
        for i in range(n_levels):
            ...
            if use_attention:
                self.down_attns.append(SelfAttention3D(out_ch, use_checkpoint=use_checkpoint))
        ...
        self.bottleneck = nn.Sequential(
            ...
            SelfAttention3D(bottleneck_ch, use_checkpoint=use_checkpoint)
            if use_attention
            else nn.Identity(),
            ResidualBlock3D(bottleneck_ch, use_checkpoint=use_checkpoint),
        )
        ...
        for i in range(n_levels):
            ...
            self.up_samples.append(nn.ConvTranspose3d(ngf * mult, ngf * mult // 2, 2, 2))
            ...
            if use_attention:
                self.up_attns.append(SelfAttention3D(out_ch, use_checkpoint=use_checkpoint))
...
class HybridGenerator2_5D(nn.Module):
    ...
    def __init__(
        ...
        self.bottleneck_3d = nn.Conv3d(ngf * 4, ngf * 4, (3, 3, 3), padding=1, bias=False)
        ...
        # final
        self.final = nn.Sequential(
            nn.ReflectionPad2d(3), nn.Conv2d(ngf, out_channels, 7, 1, 0), nn.Tanh()
        )
