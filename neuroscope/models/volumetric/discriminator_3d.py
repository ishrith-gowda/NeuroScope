...
class Discriminator3D(nn.Module):
    ...
    def __init__(
        ...
        for i in range(1, n_layers):
            ...
            conv = nn.Conv3d(ndf * mult_prev, ndf * mult, 4, 2, 1, bias=False)
            if use_spectral_norm:
                conv = SpectralNorm3d(conv)
        ...
        conv = nn.Conv3d(ndf * mult_prev, ndf * mult, 4, 1, 1, bias=False)
        if use_spectral_norm:
            conv = SpectralNorm3d(conv)
...
class NLayerDiscriminator3D(nn.Module):
    ...
    def __init__(
        ...
        # first layer
        sequence = [
            [nn.Conv3d(in_channels, ndf, 4, 2, 1, bias=True), nn.LeakyReLU(0.2, inplace=True)]
        ]
        ...
        for n in range(1, n_layers):
            ...
            conv = nn.Conv3d(ndf * nf_mult_prev, ndf * nf_mult, 4, 2, 1, bias=False)
            ...
            sequence.append([conv, norm, nn.LeakyReLU(0.2, inplace=True)])

        # penultimate layer
        nf_mult_prev = nf_mult
        nf_mult = min(2**n_layers, 8)

        conv = nn.Conv3d(ndf * nf_mult_prev, ndf * nf_mult, 4, 1, 1, bias=False)
...
class ProjectionDiscriminator3D(nn.Module):
    ...
    def __init__(
        ...
        # first layer
        layers.extend([nn.Conv3d(in_channels, ndf, 4, 2, 1), nn.LeakyReLU(0.2, inplace=True)])
...
class FeatureMatchingDiscriminator3D(nn.Module):
    ...
    def __init__(
        self, in_channels: int = 1, ndf: int = 32, n_layers: int = 4, norm_type: str = "instance"
    ):
        super().__init__()
        ...
        # first layer
        self.layers.append(
            nn.Sequential(nn.Conv3d(in_channels, ndf, 4, 2, 1), nn.LeakyReLU(0.2, inplace=True))
        )
