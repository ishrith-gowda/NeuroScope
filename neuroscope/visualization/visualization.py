...
def plot_difference_map(
    ...
    if symmetric:
        ...
        im = axes[2].imshow(diff_slice.T, cmap=cmap, vmin=-max_abs, vmax=max_abs, origin="lower")
    else:
...
def save_publication_figure(
    ...
    for fmt in formats:
        fig.savefig(f"{path}.{fmt}", format=fmt, dpi=dpi, bbox_inches="tight", pad_inches=0.1)
