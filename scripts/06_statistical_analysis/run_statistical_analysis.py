...
def run_combat_comparison(domain_a_dir: Path, domain_b_dir: Path, output_dir: Path) -> dict:
    """
...
def run_comprehensive_statistics(
    evaluation_results: dict, combat_results: dict, output_dir: Path
) -> dict:
    """
    ...
    raw_metrics = (
        evaluation_results.get("results", {}).get("domain_classification", {}).get("raw", {})
    )
    harmonized_metrics = (
        evaluation_results.get("results", {}).get("domain_classification", {}).get("harmonized", {})
    )
    ...
    feature_results = {
        ...
        * (raw_feature_stats.get("mmd", 0) - harm_feature_stats.get("mmd", 0))
        / (raw_feature_stats.get("mmd", 0) + 1e-10),
    }
    ...
    comprehensive_results = {
        ...
        "summary": {
            ...
            "combat_mmd_reduction_percent": combat_results.get("improvement", {}).get(
                "mmd_reduction_percent", 0
            ),
            "sa_cyclegan_outperforms_combat": harm_feature_stats.get("mmd", 0)
            < combat_results.get("combat", {}).get("mmd", 0),
        },
    }
...
def run_combat_on_features(
    features_a: np.ndarray, features_b: np.ndarray, output_dir: Path
) -> dict:
    """
...
def main():
    ...
    combat_results = run_combat_on_features(features_a, features_b, output_dir)
    ...
    print(
        f"  sa-cyclegan mmd reduction: {comprehensive_results['summary']['sa_cyclegan_mmd_reduction_percent']:.1f}%"
    )
    print(
        f"  combat mmd reduction: {comprehensive_results['summary']['combat_mmd_reduction_percent']:.1f}%"
    )
    print(
        f"  sa-cyclegan outperforms combat: {comprehensive_results['summary']['sa_cyclegan_outperforms_combat']}"
    )
    print("=" * 60)
