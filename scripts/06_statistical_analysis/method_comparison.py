...
def plot_method_comparison_bar(methods: List[MethodResult], output_path: Path):
    ...
    bars = ax.bar(x, accuracies, width, color=colors, edgecolor="black", linewidth=0.7, alpha=0.85)
    ...
    bars = ax.bar(x, mmds, width, color=colors, edgecolor="black", linewidth=0.7, alpha=0.85)
    ...
    bars = ax.bar(x, cosines, width, color=colors, edgecolor="black", linewidth=0.7, alpha=0.85)
    ...
    fig.suptitle(
        r"\textbf{Harmonization Method Comparison Across Evaluation Metrics}", fontsize=16, y=0.97
    )
...
def plot_improvement_waterfall(methods: List[MethodResult], output_path: Path):
    ...
    metric_labels = [r"Accuracy Reduction (\%)", r"MMD Reduction (\%)", r"Cosine Improvement (\%)"]
    ...
    fig.suptitle(
        r"\textbf{Harmonization Improvement over Unprocessed Baseline}", fontsize=16, y=0.97
    )
...
def plot_radar_comparison(methods: List[MethodResult], output_path: Path):
    ...
    for method in methods:
        ...
        ax.plot(
            ...
            "o-",
            linewidth=2,
            label=METHOD_NAMES[method.name],
    ...
    fig.suptitle(
        r"\textbf{Normalized Method Comparison Across Harmonization Metrics}",
        fontsize=16,
        x=0.58,
        y=0.97,
        ha="center",
...
def plot_statistical_summary(results: Dict, output_path: Path):
    ...
    gs_top = gridspec.GridSpec(2, 3, figure=fig, hspace=0.4, wspace=0.35, top=0.88, bottom=0.06)
    # bottom row: 7-col grid for centering 2 plots
    gs_bot = gridspec.GridSpec(2, 7, figure=fig, hspace=0.4, wspace=0.35, top=0.88, bottom=0.06)
    ...
    if "domain_classification" in results:
        ...
        raw_vals = [
            dc["raw"]["feature_statistics"]["mmd"],
            dc["raw"]["feature_statistics"]["cosine_similarity"],
        ]
        harm_vals = [
            dc["harmonized"]["feature_statistics"]["mmd"],
    ...
    if "domain_classification" in results:
        ...
        for bar, val in zip(bars, values):
            ...
            ax3.text(
                bar.get_x() + bar.get_width() / 2, ypos, f"{val:.1f}\\%", ha="center", fontsize=9
            )
    ...
    fig.suptitle(r"\textbf{Feature Distribution t-SNE Visualization}", fontsize=16, y=0.97)
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, required=True, help="output directory for figures and tables"
    )
