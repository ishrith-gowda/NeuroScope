...
def plot_domain_classification_comparison(
    ...
    if harmonized_results:
        ...
        bars1 = axes[0].bar(
            x, raw_values, width, label="Raw", color=COLORS["raw"], edgecolor="black", linewidth=0.5
        )
        for bar, val in zip(bars1, raw_values):
            axes[0].text(
                bar.get_x() + bar.get_width() / 2,
...
def plot_tsne_visualization(
    ...
    axes[0].legend(
        loc="best", frameon=True, fancybox=False, edgecolor="black", fontsize=11, markerscale=1.2
    )
    ...
    if tsne_harmonized is not None:
        axes[1].scatter(
            tsne_harmonized[mask_a, 0],
            tsne_harmonized[mask_a, 1],
            c=COLORS_TSNE["brats_harm"],
            alpha=0.6,
            s=35,
            label="BraTS (Harmonized)",
            edgecolors="black",
            linewidth=0.3,
        )
        axes[1].scatter(
            tsne_harmonized[mask_b, 0],
            tsne_harmonized[mask_b, 1],
            c=COLORS_TSNE["upenn_harm"],
            alpha=0.6,
            s=35,
            label="UPenn (Harmonized)",
            edgecolors="black",
            linewidth=0.3,
        )
    ...
    # main figure title
    fig.suptitle(
        r"\textbf{Feature Space Visualization (t-SNE)}", fontsize=16, fontweight="bold", y=0.98
    )
...
def plot_feature_distribution_metrics(
    ...
    if harmonized_metrics:
        reductions = [(r - h) / r * 100 if r > 0 else 0 for r, h in zip(raw_values, harm_values)]

        colors = [COLORS["harmonized"] if r > 0 else COLORS["raw"] for r in reductions]
        bars = axes[1].bar(
            x, reductions, width * 1.5, color=colors, edgecolor="black", linewidth=0.5
        )
...
def plot_harmonization_effect_summary(
    ...
    if "improvement" in domain_results or "improvement" in feature_results:
        ...
        colors = [
            COLORS_ENHANCED["positive"] if v > 0 else COLORS_ENHANCED["negative"]
            for v in improvements
        ]

        bars = ax3.barh(
            range(len(improvements)), improvements, color=colors, edgecolor="black", linewidth=0.7
        )
        ...
        ax3.text(
            0.5,
            0.5,
            "harmonized results not available",
    ...
    # main figure title
    fig.suptitle(r"\textbf{Harmonization Effect Summary}", fontsize=16, fontweight="bold", y=0.98)
...
def plot_training_curves(history: dict, output_path: Path):
    ...
    ax.legend(loc="lower right", frameon=True, fancybox=False, edgecolor="black", fontsize=11)
    ax.grid(True, alpha=0.3, linestyle="-", linewidth=0.5)
    ax.set_axisbelow(True)
    ...
    # main figure title
    fig.suptitle(
        r"\textbf{Domain Classifier Training Curves}", fontsize=16, fontweight="bold", y=0.98
    )
...
def create_latex_table(domain_results: dict, feature_results: dict, output_path: Path):
    ...
    for metric, label in [("accuracy", "Accuracy"), ("auc", "AUC-ROC"), ("f1", "F1-Score")]:
        ...
        if "harmonized" in domain_results:
            ...
            lines.append(
                f"{label} & {raw_val:.3f} & {harm_val:.3f} & {sign}{improvement:.1f}\\% \\\\"
            )
        else:
...
def main():
    parser = argparse.ArgumentParser(description="generate downstream evaluation figures")
    parser.add_argument(
        "--domain-results",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--tsne-dir", type=str, default=None, help="directory containing t-sne embeddings"
    )
    parser.add_argument(
        "--training-history", type=str, default=None, help="path to training history json"
    )
    parser.add_argument(
    ...
    if args.tsne_dir:
        ...
        if (tsne_dir / "tsne_raw.npy").exists():
            ...
            plot_tsne_visualization(
                tsne_raw, labels, tsne_harm, output_dir / "fig_tsne_visualization.pdf"
            )
    ...
    plot_harmonization_effect_summary(
        domain_results, feature_results, output_dir / "fig_harmonization_summary.pdf"
    )

    # latex table
    create_latex_table(domain_results, feature_results, output_dir / "table_downstream_results.tex")
