...
class DomainClassifier(nn.Module):
    ...
    def __init__(
        self, in_channels: int = 4, base_filters: int = 32, n_domains: int = 2, dropout: float = 0.3
    ):
        super().__init__()
...
class MRIDomainDataset(Dataset):
    ...
    def __init__(
        ...
        self._load_domain(self.domain_a_dir, domain_label=0, max_slices=max_slices_per_subject)
        self._load_domain(self.domain_b_dir, domain_label=1, max_slices=max_slices_per_subject)

        print(
            f"[domaindata] loaded {len(self.samples)} samples "
...
class NiftiDomainDataset(Dataset):
    ...
    def __init__(
        ...
        print(
            f"[niftidata] indexed {len(self.samples)} slices "
...
def train_domain_classifier(
    ...
    for epoch in range(n_epochs):
        ...
        print(
            f"[epoch {epoch + 1:03d}] train loss: {train_loss:.4f}, acc: {train_acc:.4f} | "
...
def main():
    ...
    parser.add_argument(
        "--domain-a-dir", type=str, required=True, help="path to domain a (brats) preprocessed data"
    )
    parser.add_argument(
        "--domain-b-dir", type=str, required=True, help="path to domain b (upenn) preprocessed data"
    )
    parser.add_argument(
        "--harmonization-model",
        type=str,
    ...
    print(
        f"[domaineval] train: {len(train_indices)}, val: {len(val_indices)}, "
    ...
    history = train_domain_classifier(
        model, train_loader, val_loader, device, n_epochs=args.n_epochs, lr=args.lr
    )
    ...
    print(
        f"[domaineval] raw data - acc: {raw_results['accuracy']:.4f}, auc: {raw_results['auc']:.4f}"
    )
    ...
    if args.harmonization_model:
        ...
        print(
            f"[domaineval] harmonized data - acc: {harmonized_results['accuracy']:.4f}, "
    ...
    if harmonized_results:
        harm_features_a = harmonized_results["features"][
            np.array(harmonized_results["labels"]) == 0
        ]
        harm_features_b = harmonized_results["features"][
            np.array(harmonized_results["labels"]) == 1
        ]
