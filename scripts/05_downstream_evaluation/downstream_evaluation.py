...
class SegmentationDataset(Dataset):
    ...
    def __init__(self, data_dir: Path, subjects: list[str], transform: Optional[callable] = None):
    ...
    if np.any(pred) and np.any(target):
        ...
        if len(pred_points) > 0 and len(target_points) > 0:
            ...
            if len(dist_pred_to_target) > 0 and len(dist_target_to_pred) > 0:
                hd95 = max(
                    np.percentile(dist_pred_to_target, 95), np.percentile(dist_target_to_pred, 95)
                )
            else:
                hd95 = float("inf")
        else:
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc=f"epoch {epoch + 1}/{epochs} [val]"):
                ...
                pred_tc = (
                    ((pred[:, 1] + pred[:, 3]) > 0.5).float() if pred.shape[1] > 3 else pred_wt
                )
        ...
        mean_dice = (
            history["val_dice_wt"][-1] + history["val_dice_tc"][-1] + history["val_dice_et"][-1]
        ) / 3
...
def statistical_comparison(
    baseline_results: dict, harmonized_results: dict, metric_keys: list[str]
) -> dict:
    """
...
def main():
    parser = argparse.ArgumentParser(description="downstream task evaluation for mri harmonization")
    parser.add_argument(
        "--seg-data-dir", type=str, required=True, help="path to preprocessed segmentation data"
    )
    parser.add_argument(
        "--harmonization-checkpoint", type=str, help="path to sa-cyclegan checkpoint"
    )
    parser.add_argument(
        "--baseline-checkpoint", type=str, help="path to baseline cyclegan checkpoint"
    )
    parser.add_argument(
    ...
    parser.add_argument("--epochs", type=int, default=50, help="epochs for segmentation training")
    parser.add_argument("--lr", type=float, default=1e-4)
    parser.add_argument(
        "--skip-training", action="store_true", help="skip training and use existing checkpoint"
    )
    parser.add_argument(
        "--seg-checkpoint", type=str, help="existing segmentation checkpoint to use"
    )
    ...
    with open(seg_data_dir / "splits.json") as f:
        splits = json.load(f)
    ...
    print(
        f"  dice_wt: {raw_results['dice_wt']['mean']:.4f} +/- {raw_results['dice_wt']['std']:.4f}"
    )
    print(
        f"  dice_tc: {raw_results['dice_tc']['mean']:.4f} +/- {raw_results['dice_tc']['std']:.4f}"
    )
    print(
        f"  dice_et: {raw_results['dice_et']['mean']:.4f} +/- {raw_results['dice_et']['std']:.4f}"
    )
    ...
    if args.harmonization_checkpoint:
        ...
        checkpoint = torch.load(
            args.harmonization_checkpoint, map_location=device, weights_only=False
        )
        ...
        harmonized_results = evaluate_segmentation(seg_model, test_loader, device, harm_model)
        ...
        print(
            f"  dice_wt: {harmonized_results['dice_wt']['mean']:.4f} +/- {harmonized_results['dice_wt']['std']:.4f}"
        )
        print(
            f"  dice_tc: {harmonized_results['dice_tc']['mean']:.4f} +/- {harmonized_results['dice_tc']['std']:.4f}"
        )
        print(
            f"  dice_et: {harmonized_results['dice_et']['mean']:.4f} +/- {harmonized_results['dice_et']['std']:.4f}"
        )
        ...
        for key, stats in comparison.items():
            ...
            print(
                f"  {key}: {stats['improvement_pct']:+.2f}%{sig} "
