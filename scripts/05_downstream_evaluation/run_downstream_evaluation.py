...
def main():
    parser = argparse.ArgumentParser(description="run complete downstream evaluation pipeline")
    parser.add_argument(
    ...
    parser.add_argument(
        "--harmonization-model", type=str, default=None, help="path to trained harmonization model"
    )
    parser.add_argument("--output-dir", type=str, required=True, help="output directory")
    parser.add_argument(
        "--n-epochs", type=int, default=50, help="domain classifier training epochs"
    )
    parser.add_argument("--batch-size", type=int, default=32, help="batch size")
    parser.add_argument("--device", type=str, default="cuda", help="device")
    parser.add_argument(
        "--skip-classification", action="store_true", help="skip domain classification"
    )
    parser.add_argument(
        "--skip-features", action="store_true", help="skip feature distribution analysis"
    )
    ...
    if not args.skip_classification:
        ...
        cmd = [
            sys.executable,
            str(script_dir / "domain_classifier.py"),
            "--domain-a-dir",
            args.domain_a_dir,
            "--domain-b-dir",
            args.domain_b_dir,
    ...
    if not args.skip_features:
        ...
        cmd = [
            sys.executable,
            str(script_dir / "feature_distribution_analysis.py"),
            "--domain-a-dir",
            args.domain_a_dir,
            "--domain-b-dir",
            args.domain_b_dir,
    ...
    if not args.skip_figures:
        ...
        if domain_results.exists() and feature_results.exists():
            cmd = [
                sys.executable,
                str(script_dir / "generate_downstream_figures.py"),
                "--domain-results",
                str(domain_results),
                "--feature-results",
                str(feature_results),
    ...
    summary = {"timestamp": timestamp, "configuration": config, "results": {}}
