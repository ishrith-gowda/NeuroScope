...
def create_ablation_figure(results: dict, output_path: Path):
    ...
    for i, key in enumerate(sig_keys):
        if stats[key]["significant"]:
            max_val = max(
                baseline_vals[i] + baseline_stds[i], attention_vals[i] + attention_stds[i]
            )
    ...
    for i, key in enumerate(["cycle_psnr_A", "cycle_psnr_B"]):
        if stats[key]["significant"]:
            max_val = max(
                baseline_psnr[i] + baseline_psnr_std[i], attention_psnr[i] + attention_psnr_std[i]
            )
    ...
    for i in range(len(modalities)):
        ...
        if improvement > 0:
            ax.annotate(
                f"+{improvement:.1f}\\%",
                xy=(i + width / 2, attention_mod[i] + 0.003),
                ha="center",
                fontsize=10,
                color="green",
            )
    ...
    # main figure title
    fig.suptitle(
        r"\textbf{Ablation Study: Self-Attention Impact on Harmonization Quality}",
        fontsize=15,
        y=0.97,
    )
...
\multirow{2}{*}{a $\rightarrow$ b $\rightarrow$ a}
  & cycle ssim & """
        + f"{stats['cycle_ssim_A']['difference'] * 100:+.2f}"
        + r"""\% & """
...
\multirow{2}{*}{b $\rightarrow$ a $\rightarrow$ b}
  & cycle ssim & """
        + f"{stats['cycle_ssim_B']['difference'] * 100:+.2f}"
        + r"""\% & """
...
def main():
    parser = argparse.ArgumentParser(description="generate ablation study publication figure")
    parser.add_argument("--results", type=str, required=True, help="path to ablation results json")
    parser.add_argument(
