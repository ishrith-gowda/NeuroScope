            ...
            if img1.size(1) == 4:
                ...
                for c in range(4):
                    score = self.lpips_model(
                        img1[:, c : c + 1].repeat(1, 3, 1, 1), img2[:, c : c + 1].repeat(1, 3, 1, 1)
                    )
                ...
                return (
                    lpips_scores.tolist()
                    if hasattr(lpips_scores, "tolist")
                    else [float(lpips_scores)]
                )
            else:
            ...
            for batch_idx, batch in enumerate(tqdm(test_loader, desc="evaluating")):
                ...
                for i in range(center_a.size(0)):
                    ...
                    if batch_idx == 0 and i == 0:
                        logger.info(
                            f"debug shapes - ca: {ca_np.shape}, cb: {cb_np.shape}, fa: {fa_np.shape}, fb: {fb_np.shape}"
                        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="comprehensive evaluation of sa-cyclegan-2.5d")

    parser.add_argument(
        "--checkpoint", type=str, required=True, help="path to trained model checkpoint"
    )

    parser.add_argument("--brats-dir", type=str, required=True, help="brats dataset directory")

    parser.add_argument("--upenn-dir", type=str, required=True, help="upenn dataset directory")
    ...
    parser.add_argument(
        "--device", type=str, default="cuda", choices=["cuda", "cpu"], help="computation device"
    )
