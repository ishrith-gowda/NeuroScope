    ...
    def anova_with_posthoc(self, data_dict: dict[str, np.ndarray]) -> tuple[float, float, dict]:
        ...
        for i in range(len(model_names)):
            for j in range(i + 1, len(model_names)):
                ...
                # paired t-test
                t_stat, p_val = self.paired_t_test(data_dict[model1], data_dict[model2])

                # effect size
                effect_size = self.cohens_d(data_dict[model1], data_dict[model2])
        ...
        for other_model in model_names:
            if other_model != "sa_cyclegan":
                pairwise_table = analyzer.generate_pairwise_table(
                    analyses, "sa_cyclegan", other_model
                )
