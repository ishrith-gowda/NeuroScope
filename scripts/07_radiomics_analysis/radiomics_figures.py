...
def plot_correlation_heatmap(
    ...
    heatmap_data = np.column_stack(
        [all_cccs[sorted_indices], all_iccs[sorted_indices], all_corrs[sorted_indices]]
    )
    ...
    fig.suptitle(r"\textbf{Radiomics Feature Preservation Heatmap}", fontsize=15, y=0.97)
    fig.subplots_adjust(top=0.90)

    im = ax.imshow(
        heatmap_data, aspect="auto", cmap="RdBu_r", vmin=-vmax, vmax=vmax, interpolation="nearest"
    )
    ...
    for i, (cat_name, start) in enumerate(zip(category_labels, category_boundaries)):
            ...
            category_boundaries[i + 1] if i + 1 < len(category_boundaries) else len(sorted_indices)
        )
        mid = (start + end) / 2
        ax.text(
            -0.3,
...
def plot_preservation_by_category(
    ...
    fig.suptitle(r"\textbf{Radiomics Feature Preservation by Category}", fontsize=15, y=0.97)
    ...
    metric_data = [
        ...
        (
            r"(c) Pearson $r$ by Feature Category",
    ...
    for ax, (title, ylabel, means, stds) in zip(axes, metric_data):
        bars = ax.bar(
            x, means, width, yerr=stds, capsize=4, color=colors, edgecolor="black", linewidth=0.5
        )
...
def plot_bland_altman(
    ...
    fig.suptitle(
        r"\textbf{Bland--Altman Analysis of Radiomics Feature Preservation}", fontsize=15, y=0.97
    )
    ...
    for idx, (ax, feature_idx) in enumerate(zip(axes, selected_indices)):
        ...
        ax.axhline(
            y=mean_diff, color=COLORS["secondary"], linestyle="-", label=f"Mean: {mean_diff:.3f}"
        )
        ax.axhline(
            y=loa_upper,
...
def plot_preservation_scatter(
    ...
    fig, axes = plt.subplots(2, 2, figsize=(13, 10))
    fig.suptitle(
        r"\textbf{Original vs.\ Harmonized Radiomics Feature Scatter Plots}", fontsize=15, y=0.97
    )
    fig.subplots_adjust(top=0.90, hspace=0.30, wspace=0.28)
    axes = axes.flatten()
    ...
    for idx, (ax, feature_idx) in enumerate(zip(axes, selected_indices)):
        ...
        ax.plot(
            x_line, y_line, color=COLORS["secondary"], linewidth=2, label=f"$r = {r_value:.3f}$"
        )

        # identity line
        ax.plot(
            x_line, x_line, color=COLORS["neutral"], linestyle="--", alpha=0.5, label="Identity"
        )
...
def plot_comprehensive_summary(
    ...
    bars = ax1.bar(
        metrics, values, yerr=errors, capsize=5, color=colors, edgecolor="black", linewidth=0.5
    )
    ...
    ax2.bar(quality_labels, quality_counts, color=quality_colors, edgecolor="black", linewidth=0.5)
...
def main():
    parser = argparse.ArgumentParser(description="generate radiomics preservation figures")
    parser.add_argument(
        "--results", type=str, required=True, help="path to preservation results json"
    )
    parser.add_argument("--original-a", type=str, help="path to original domain a features")
    parser.add_argument("--harmonized-a", type=str, help="path to harmonized domain a features")
    ...
    parser.add_argument(
        "--method-name", type=str, default="SA-CycleGAN-2.5D", help="name of harmonization method"
    )
    ...
    # correlation heatmap
    plot_correlation_heatmap(domain_a_results, output_dir / "fig_radiomics_correlation_heatmap.pdf")

    # preservation by category
    plot_preservation_by_category(
        domain_a_results, output_dir / "fig_radiomics_preservation_category.pdf", args.method_name
    )

    # comprehensive summary
    plot_comprehensive_summary(
        results, output_dir / "fig_radiomics_comprehensive.pdf", args.method_name
    )
    ...
    if args.original_a and args.harmonized_a:
        ...
        plot_bland_altman(
            original, harmonized, feature_names, output_dir / "fig_radiomics_bland_altman.pdf"
        )

        plot_preservation_scatter(
            original, harmonized, feature_names, output_dir / "fig_radiomics_scatter.pdf"
        )

    # latex table
    create_latex_table(domain_a_results, output_dir / "table_radiomics_preservation.tex")
