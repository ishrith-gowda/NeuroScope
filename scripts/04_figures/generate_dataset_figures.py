...
def generate_dataset_statistics_figure():
    ...
    bars = ax.bar(
        datasets, samples, color=colors_list, alpha=0.8, width=0.6, edgecolor="black", linewidth=0.7
    )
    ...
    bars = ax.bar(
        components, values, color=bar_colors, alpha=0.8, width=0.6, edgecolor="black", linewidth=0.7
    )
    ...
    fig.suptitle(r"\textbf{Dataset Statistics and Experimental Setup}", fontsize=16, y=0.98)
...
def generate_preprocessing_pipeline_figure():
    ...
    # title
    fig.suptitle(
        r"\textbf{Preprocessing Pipeline: Raw MRI to 2.5D Slice Triplets}", fontsize=16, y=0.98
    )
    ...
    for i, stage in enumerate(stages):
        ...
        # text
        ax.text(
            2.5, stage["y"], stage["name"], ha="center", va="center", fontsize=10, fontweight="bold"
        )
...
def generate_25d_processing_figure():
    ...
    fig.suptitle(r"\textbf{2.5D Processing: Context-Aware Slice Translation}", fontsize=16, y=0.98)
    ...
    gen_rect = mpatches.FancyBboxPatch(
        (5, input_y - 0.2),
        2,
        1.4,
        boxstyle="round,pad=0.1",
    ...
    ax.text(6, input_y + 0.1, r"(with attention)", ha="center", va="center", fontsize=10)
    ...
    output_x = 8.5
    out_rect = mpatches.Rectangle(
        (output_x, input_y - 0.1),
        1,
        1.2,
        facecolor="#CC6644",
        edgecolor="black",
...
def generate_training_overview_figure():
    ...
    fig.suptitle(r"\textbf{SA-CycleGAN-2.5D Training Configuration}", fontsize=16, y=0.98)
