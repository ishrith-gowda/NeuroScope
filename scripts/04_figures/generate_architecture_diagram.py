...
def draw_block(ax, x, y, width, height, color, label, fontsize=7, edgecolor="black"):
    ...
    ax.add_patch(rect)
    ax.text(
        x, y, label, ha="center", va="center", fontsize=fontsize, color=COLORS["text"], wrap=True
    )
    return rect
...
def create_generator_diagram(output_dir: Path):
    ...
    ax.text(
        7,
        8.5,
        "SA-CycleGAN-2.5D Generator Architecture",
        fontsize=14,
        ha="center",
        fontweight="bold",
    )
    ...
    # input block
    draw_block(
        ax, 0.5, 5, 1.2, 2.5, COLORS["input"], "Input\n3×4×H×W\n(3 slices\n4 mods)", fontsize=7
    )
    ...
    # downsample blocks
    draw_block(ax, 3.8, 5, 1.2, 2.0, COLORS["conv"], "Down ×2\n3×3 Conv\nstride=2", fontsize=7)

    draw_block(ax, 5.2, 5, 1.2, 2.0, COLORS["conv"], "Down ×2\n3×3 Conv\nstride=2", fontsize=7)
    ...
    draw_block(ax, 10.2, 5, 1.5, 2.0, COLORS["residual"], "ResBlocks\n4-9\n(+CBAM)", fontsize=7)

    # upsample blocks
    draw_block(ax, 11.6, 5, 1.2, 2.0, COLORS["upsample"], "Up ×2\nTransConv\nstride=2", fontsize=7)

    draw_block(ax, 12.8, 5, 1.2, 2.0, COLORS["upsample"], "Up ×2\nTransConv\nstride=2", fontsize=7)

    # output
    draw_block(
        ax, 14.2, 5, 1.2, 2.5, COLORS["output"], "Output\n4×H×W\n(center\nslice)", fontsize=7
    )
    ...
    ax.legend(handles=legend_elements, loc="lower left", ncol=3, fontsize=8, framealpha=0.9)
...
def create_full_cyclegan_diagram(output_dir: Path):
    ...
    ax.text(
        5.75,
        6.7,
        "SA-CycleGAN-2.5D: Bidirectional MRI Harmonization",
        fontsize=14,
        ha="center",
        fontweight="bold",
    )
    ...
    # loss labels
    ax.text(
        10.5, 3.5, "Adversarial\nLoss", fontsize=8, ha="center", color="#D32F2F", fontweight="bold"
    )
    ...
    ax.text(
        5.75,
        0.5,
        "L_cycle = ||G_B→A(G_A→B(A)) - A|| + ||G_A→B(G_B→A(B)) - B||",
        fontsize=8,
        ha="center",
        family="monospace",
    )
    ...
    legend_elements = [
        ...
        mpatches.Patch(
            facecolor=COLORS["attention"], edgecolor="black", label="Generator (w/ Attention)"
        ),
        mpatches.Patch(facecolor=COLORS["discriminator"], edgecolor="black", label="Discriminator"),
    ]
    ax.legend(handles=legend_elements, loc="lower left", ncol=2, fontsize=8, framealpha=0.9)
...
def create_25d_concept_diagram(output_dir: Path):
    ...
    # title
    ax.text(5, 4.7, "2.5D Slice Processing Concept", fontsize=14, ha="center", fontweight="bold")
    ...
    # stacked input
    draw_block(ax, 5, 2.5, 2, 2, COLORS["conv"], "2.5D Input\n[3×4, H, W]\n12 channels", fontsize=9)

    # arrow
    draw_arrow(ax, (6.2, 2.5), (7.2, 2.5), COLORS["arrow"])

    # generator
    draw_block(ax, 8, 2.5, 1.5, 1.8, COLORS["attention"], "Generator\nG_A→B", fontsize=9)

    # arrow
    draw_arrow(ax, (8.9, 2.5), (9.8, 2.5), COLORS["arrow"])

    # output
    rect = Rectangle((9.8, 1.5), 1.5, 2, facecolor=COLORS["output"], edgecolor="black", linewidth=1)
    ax.add_patch(rect)
    ax.text(10.55, 2.5, "Output\n[4, H, W]\nCenter\nSlice", fontsize=8, ha="center")

    # modality labels
    ax.text(5, 0.8, "4 modalities: T1, T1CE, T2, FLAIR", fontsize=8, ha="center", style="italic")
