...
def generate_architecture_comparison():
    ...
    for block in baseline_blocks:
        ...
        ax.add_patch(rect)
        ax.text(2.5, block["y"], block["name"], ha="center", va="center", fontsize=10)
    ...
    for block in sa_blocks:
        ...
        ax.add_patch(rect)
        ax.text(2.5, block["y"], block["name"], ha="center", va="center", fontsize=9)
    ...
    legend_elements = [
        ...
        mpatches.Patch(
            facecolor=c_attention, alpha=0.7, edgecolor="red", linewidth=2, label="Self-Attention"
        ),
    ]
    fig.legend(
    ...
    fig.suptitle(r"\textbf{Model Architecture Comparison}", fontsize=16, y=0.98)
...
def generate_attention_mechanism_diagram():
    ...
    for i, block in enumerate(cbam_blocks):
        ...
        ax.add_patch(rect)
        ax.text(block["x"], block["y"], block["name"], ha="center", va="center", fontsize=10)
    ...
    ax.add_patch(rect)
    ax.text(3, sa_y, r"Attention = softmax$(QK^T/\sqrt{d})V$", ha="center", va="center", fontsize=9)
    ...
    fig.suptitle(r"\textbf{Attention Mechanisms in SA-CycleGAN-2.5D}", fontsize=16, y=0.98)
...
def generate_parameter_breakdown():
    ...
    bars = ax.barh(models, params, color=bar_colors, height=0.65, edgecolor="black", linewidth=0.5)
    ax.set_ylim(-0.5, 1.5)
    ...
    ax.text(
        0.98,
        0.05,
        r"+" + f"{diff:.2f}M (+{pct_increase:.1f}" + r"\%) for attention",
    ...
    fig.savefig(
        OUTPUT_DIR / "fig14_parameter_breakdown.pdf",
        format="pdf",
        bbox_inches="tight",
        pad_inches=0.15,
    )
...
def generate_cyclegan_workflow():
    ...
    fig.suptitle(r"\textbf{CycleGAN Training Workflow}", fontsize=16, y=0.98)
    ...
    rect_a = mpatches.Rectangle(
        (1, a_y - 0.4), 2, 0.8, facecolor=c_dom_a, edgecolor="black", alpha=0.6, linewidth=2
    )
    ax.add_patch(rect_a)
    ...
    rect_b = mpatches.Rectangle(
        (9, b_y - 0.4), 2, 0.8, facecolor=c_dom_b, edgecolor="black", alpha=0.6, linewidth=2
    )
    ax.add_patch(rect_b)
    ax.text(
        10, b_y, r"Real $B$\\ (UPenn)", ha="center", va="center", fontsize=10, fontweight="bold"
    )

    # generator a→b
    gen_ab_y = 6.5
    rect_gab = mpatches.FancyBboxPatch(
        (4, gen_ab_y - 0.4),
        2,
        0.8,
    ...
    # generator b→a
    gen_ba_y = 4.5
    rect_gba = mpatches.FancyBboxPatch(
        (4, gen_ba_y - 0.4),
        2,
        0.8,
        boxstyle="round,pad=0.1",
    ...
    # discriminators
    disc_a_y = 3
    rect_da = mpatches.Ellipse(
        (1.5, disc_a_y), 1.2, 0.6, facecolor=c_disc, edgecolor="black", alpha=0.6, linewidth=1.5
    )
    ax.add_patch(rect_da)
    ax.text(1.5, disc_a_y, r"$D_A$", ha="center", va="center", fontsize=11, fontweight="bold")

    disc_b_y = 3
    rect_db = mpatches.Ellipse(
        (9.5, disc_b_y), 1.2, 0.6, facecolor=c_disc, edgecolor="black", alpha=0.6, linewidth=1.5
    )
