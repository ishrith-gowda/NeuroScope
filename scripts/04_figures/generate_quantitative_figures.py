...
def generate_cycle_consistency_figure(cycle_results):
    ...
    fig.suptitle(r"\textbf{Cycle Consistency Reconstruction Quality}", fontsize=16, y=0.97)
...
def generate_quantitative_tables(eval_results, cycle_results):
    ...
    table1.append(
        r"Metric & $A \rightarrow B$ (BraTS $\rightarrow$ UPenn) & $B \rightarrow A$ (UPenn $\rightarrow$ BraTS) \\"
    )
    table1.append(r"\hline")
    ...
    for name, key, note in metrics:
        if key == "fid":
            ...
            table1.append(
                f"{name} & {mean_a2b:.4f} $\\pm$ {std_a2b:.4f} & {mean_b2a:.4f} $\\pm$ {std_b2a:.4f} \\\\"
            )
    ...
    table2.append(
        r"Metric & Cycle A ($A \rightarrow B \rightarrow A$) & Cycle B ($B \rightarrow A \rightarrow B$) \\"
    )
    table2.append(r"\hline")
    ...
    for name, key in [("SSIM", "ssim"), ("PSNR (dB)", "psnr"), ("MAE", "mae")]:
        ...
        table2.append(
            f"{name} & {mean_a:.4f} $\\pm$ {std_a:.4f} & {mean_b:.4f} $\\pm$ {std_b:.4f} \\\\"
        )
