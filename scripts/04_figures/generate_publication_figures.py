...
def create_training_curves_figure(
    baseline_history: dict, attention_history: dict, output_dir: Path, smooth: bool = True
) -> None:
    """
    ...
    for ax, (metric, title, split) in zip(axes.flat, metrics):
        ...
        if smooth and metric != "val_ssim":
            ...
            # plot smoothed
            ax.plot(
                x, baseline_smooth, color=COLORS["baseline"], label="Baseline CycleGAN", linewidth=2
            )
            ax.plot(
                x,
                attention_smooth,
                color=COLORS["attention"],
                label="SA-CycleGAN (Ours)",
                linewidth=2,
            )
        else:
            ax.plot(
                x, baseline_data, color=COLORS["baseline"], label="Baseline CycleGAN", linewidth=1.5
            )
            ax.plot(
                x,
                attention_data,
...
def create_ablation_bar_chart(ablation_results: dict, output_dir: Path) -> None:
    ...
    for i, m in enumerate(metrics):
        if stats[m]["significant"]:
            max_val = max(
                baseline_means[i] + baseline_stds[i], attention_means[i] + attention_stds[i]
            )
...
def create_radar_chart(ablation_results: dict, output_dir: Path) -> None:
    ...
    for ax, direction, title in zip(
        axes, ["A", "B"], ["(a) A→B→A Direction", "(b) B→A→B Direction"]
    ):
        ...
        ax.plot(
            angles, baseline_norm, "o-", linewidth=2, label="Baseline", color=COLORS["baseline"]
        )
...
def create_summary_table_figure(ablation_results: dict, output_dir: Path) -> None:
    ...
    rows = [
        [
            "Cycle SSIM (A→B→A)",
            f"{stats['cycle_ssim_A']['baseline_mean']:.4f}±{stats['cycle_ssim_A']['baseline_std']:.4f}",
            f"{stats['cycle_ssim_A']['attention_mean']:.4f}±{stats['cycle_ssim_A']['attention_std']:.4f}",
            f"{stats['cycle_ssim_A']['difference']:+.4f}",
            "<0.001***",
            f"{stats['cycle_ssim_A']['cohens_d']:.2f}",
        ],
        [
            "Cycle SSIM (B→A→B)",
            f"{stats['cycle_ssim_B']['baseline_mean']:.4f}±{stats['cycle_ssim_B']['baseline_std']:.4f}",
            f"{stats['cycle_ssim_B']['attention_mean']:.4f}±{stats['cycle_ssim_B']['attention_std']:.4f}",
            f"{stats['cycle_ssim_B']['difference']:+.4f}",
            "<0.001***",
            f"{stats['cycle_ssim_B']['cohens_d']:.2f}",
        ],
        [
            "Cycle PSNR (A→B→A)",
            f"{stats['cycle_psnr_A']['baseline_mean']:.2f}±{stats['cycle_psnr_A']['baseline_std']:.2f}",
            f"{stats['cycle_psnr_A']['attention_mean']:.2f}±{stats['cycle_psnr_A']['attention_std']:.2f}",
            f"{stats['cycle_psnr_A']['difference']:+.2f}",
            "<0.001***",
            f"{stats['cycle_psnr_A']['cohens_d']:.2f}",
        ],
        [
            "Cycle PSNR (B→A→B)",
            f"{stats['cycle_psnr_B']['baseline_mean']:.2f}±{stats['cycle_psnr_B']['baseline_std']:.2f}",
            f"{stats['cycle_psnr_B']['attention_mean']:.2f}±{stats['cycle_psnr_B']['attention_std']:.2f}",
            f"{stats['cycle_psnr_B']['difference']:+.2f}",
            "<0.001***",
            f"{stats['cycle_psnr_B']['cohens_d']:.2f}",
        ],
    ]
    ...
    ax.set_title(
        "Table 1: Ablation Study Results - Quantitative Comparison\n(*** indicates p < 0.001)",
        fontsize=12,
        fontweight="bold",
        pad=20,
    )
...
def main():
    ...
    parser.add_argument(
        "--ablation-results", type=str, required=True, help="path to ablation results json"
    )
    parser.add_argument(
        "--baseline-history", type=str, required=True, help="path to baseline training history json"
    )
    parser.add_argument(
        "--attention-history",
        type=str,
