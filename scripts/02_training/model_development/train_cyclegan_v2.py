...
class ResNetGenerator(nn.Module):
    def __init__(self, in_ch=4, out_ch=4, n_residual=9, ngf=64):
        ...
        model += [nn.ReflectionPad2d(3), nn.Conv2d(ngf, out_ch, kernel_size=7), nn.Tanh()]
...
def train(args, device: torch.device):
    ...
    opt_G = optim.Adam(
        itertools.chain(G_A2B.parameters(), G_B2A.parameters()), lr=lr_G, betas=(0.5, 0.999)
    )
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        for i in range(min(len(train_loader_A), len(train_loader_B))):
            ...
            torch.nn.utils.clip_grad_norm_(
                itertools.chain(G_A2B.parameters(), G_B2A.parameters()), max_norm=5.0
            )
            opt_G.step()
            ...
            if global_step % 10 == 0:
                ...
                tb_writer.add_scalar(
                    "Loss/Cycle", (loss_cycle_A + loss_cycle_B).item(), global_step
                )
        ...
        if epoch % args.checkpoint_interval == 0:
            torch.save(
                G_A2B.state_dict(), os.path.join(args.checkpoint_dir, f"G_A2B_v2_{epoch}.pth")
            )
            torch.save(
                G_B2A.state_dict(), os.path.join(args.checkpoint_dir, f"G_B2A_v2_{epoch}.pth")
            )
    ...
    # final saves
    torch.save(
        G_A2B.state_dict(), os.path.join(args.checkpoint_dir, "final_models", "G_A2B_v2_final.pth")
    )
    torch.save(
        G_B2A.state_dict(), os.path.join(args.checkpoint_dir, "final_models", "G_B2A_v2_final.pth")
    )
    torch.save(
        D_A.state_dict(), os.path.join(args.checkpoint_dir, "final_models", "D_A_v2_final.pth")
    )
    torch.save(
        D_B.state_dict(), os.path.join(args.checkpoint_dir, "final_models", "D_B_v2_final.pth")
    )
