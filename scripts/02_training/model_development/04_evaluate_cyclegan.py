...
def evaluate(
    ...
    loaders = get_cycle_domain_loaders(
        data_root, meta_json, batch_size=batch_size, num_workers=0, slices_per_subject=4
    )
    key = f"{split}_A"
        ...
        for bidx, real in enumerate(dl, 1):
            ...
            if bidx == 1:
                grid = vutils.make_grid(
                    torch.cat([(real + 1) / 2.0, (fake + 1) / 2.0], dim=0),
                    nrow=real.size(0),
                    normalize=True,
                )
    ...
    logging.info(
        "wrote evaluation report to %s", os.path.join(output_dir, "evaluation_report.json")
    )


def parse_args():
    ap = argparse.ArgumentParser(
        description="Evaluate CycleGAN generator with SSIM/PSNR and samples"
    )
