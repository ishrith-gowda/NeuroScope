...
class PublicationFigureGenerator:
    ...
    def generate_loss_curves(
        ...
        if identity:
            ax.fill_between(
                range(len(identity)), identity, alpha=0.2, color=self.colors["identity"]
            )
    ...
    def generate_metrics_chart(self, metrics: dict):
        ...
        ax.bar(
            ...
            yerr=ssim_stds,
            capsize=5,
            color=[self.colors["brats"], self.colors["upenn"]],
            edgecolor="black",
        )
        ax.set_ylabel("SSIM")
    ...
    def generate_dataset_distribution(self, metadata: dict):
        ...
        brats_splits = [
            metadata.get("brats_train", 62),
            metadata.get("brats_val", 14),
            metadata.get("brats_test", 12),
        ]
        upenn_splits = [
            metadata.get("upenn_train", 403),
    ...
    def generate_config_table(self, config: dict):
        ...
        table_data = [
            ...
            [
                "Learning Rate (D)",
                str(config.get("lr_d", "5e-5")),
                "Discriminator learning rate (TTUR)",
            ],
    ...
    def generate_epoch_metrics(self, epoch_metrics: dict):
        ...
        ax = axes[0]
        ax.plot(
            epochs, ssim_a2b, "o-", color=self.colors["brats"], lw=2, markersize=6, label="A→B→A"
        )
        ax.plot(
            epochs, ssim_b2a, "s-", color=self.colors["upenn"], lw=2, markersize=6, label="B→A→B"
        )
        ...
        ax = axes[1]
        ax.plot(
            epochs, psnr_a2b, "o-", color=self.colors["brats"], lw=2, markersize=6, label="A→B→A"
        )
        ax.plot(
            epochs, psnr_b2a, "s-", color=self.colors["upenn"], lw=2, markersize=6, label="B→A→B"
        )
...
def main():
    ...
    parser.add_argument(
        "--output_dir", type=str, default="/Volumes/usb drive/neuroscope/figures/publication"
    )
    parser.add_argument(
        "--loss_file",
        type=str,
        default="/Volumes/usb drive/neuroscope/samples/training_loss_log.json",
    )
    parser.add_argument(
