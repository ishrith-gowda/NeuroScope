...
class CachedSliceDataset(Dataset):
    ...
    def __init__(
        ...
        subject_ids = [sid for sid, info in subjects_meta.items() if info.get("split") == split]
        ...
        for idx, sid in enumerate(subject_ids):
            ...
            try:
                ...
                if (idx + 1) % 20 == 0:
                    pprint(
                        f"  loaded {idx + 1}/{len(subject_ids)} subjects, {len(self.slices)} slices"
                    )
...
def train(args):
    ...
    loader_A = DataLoader(
        dataset_A, batch_size=args.batch_size, shuffle=True, num_workers=0, drop_last=True
    )
    loader_B = DataLoader(
        dataset_B, batch_size=args.batch_size, shuffle=True, num_workers=0, drop_last=True
    )
    ...
    opt_D = optim.Adam(
        itertools.chain(D_A.parameters(), D_B.parameters()), lr=args.lr * 0.5, betas=(0.5, 0.999)
    )
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        for i in range(n_iters):
            ...
            loss_G = (
                loss_GAN_A2B + loss_GAN_B2A + loss_cycle_A + loss_cycle_B + loss_id_A + loss_id_B
            )
            ...
            if (i + 1) % args.log_interval == 0 or i == 0:
                pprint(
                    f"  [{epoch}/{args.n_epochs}] [{i + 1}/{n_iters}] "
        ...
        if epoch % args.checkpoint_interval == 0:
            torch.save(
                G_A2B.state_dict(), os.path.join(args.checkpoint_dir, f"fast_G_A2B_{epoch}.pth")
            )
            torch.save(
                G_B2A.state_dict(), os.path.join(args.checkpoint_dir, f"fast_G_B2A_{epoch}.pth")
            )
            torch.save(D_A.state_dict(), os.path.join(args.checkpoint_dir, f"fast_D_A_{epoch}.pth"))
            torch.save(D_B.state_dict(), os.path.join(args.checkpoint_dir, f"fast_D_B_{epoch}.pth"))
    ...
    pprint(
        f"final losses - g: {loss_history['G_total'][-1]:.4f}, "
...
def main():
    ...
    parser.add_argument(
        "--max_subjects", type=int, default=50, help="Max subjects to load per domain (for speed)"
    )
