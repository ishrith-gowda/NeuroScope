...
def list_subjects_for_split(metadata_path: Path, split: str) -> dict[str, list[str]]:
    ...
    logging.info(
        f"Found {len(out['brats'])} BRATS subjects and {len(out['upenn'])} UPENN subjects for split '{split}' in {elapsed:.2f}s"
    )
    return out
...
def build_manifest(splits: list[str], verbose: bool = False) -> dict:
    ...
    for split in splits:
        ...
        for section in ("brats", "upenn"):
            ...
            logging.info(
                f"Processing {section_total} subjects from {section} dataset for {split} split"
            )
            ...
            for i, sid in enumerate(subjects[section]):
                ...
                if ok:
                    ...
                    manifest["splits"][split][section].append(
                        {"subject": sid, "depth": depth, "path": str(pre_dir / section / sid)}
                    )

            section_time = time.time() - section_start
            logging.info(
                f"Completed {section} for {split}: {section_valid}/{section_total} valid subjects in {section_time:.2f}s"
            )
    ...
    for split in splits:
        summary[split] = {
            ...
            "total_subjects": len(manifest["splits"][split]["brats"])
            + len(manifest["splits"][split]["upenn"]),
        }
    ...
    logging.info(
        f"Manifest building complete: {total_subjects_valid}/{total_subjects_processed} valid subjects in {total_time:.2f}s"
    )
...
def parse_args():
    ...
    ap.add_argument(
        "--splits", type=str, default="train,val", help="Comma list of splits to include"
    )
