...
def validate_preprocessing_outputs() -> dict[str, any]:
    ...
    results = {"validated_subjects": 0, "failed_subjects": [], "tensor_ranges": [], "issues": []}
            ...
            for modality in modalities:
                ...
                if not modality_path.exists():
                    results["failed_subjects"].append(
                        f"{section}/{subject_id} (missing {modality})"
                    )
                    subject_valid = False
                    continue
                ...
                try:
                    ...
                    if not np.isfinite(arr).all():
                        results["issues"].append(
                            f"{section}/{subject_id}/{modality} contains non-finite values"
                        )
                        subject_valid = False
                        continue
...
def validate_data_consistency() -> dict[str, any]:
    ...
    results = {"consistency_checks": [], "issues": []}
        ...
        for section, subject_id in sample_subjects.items():
            ...
            if modality_path.exists():
                ...
                if loader_key in loaders:
                    ...
                    results["consistency_checks"].append(
                        {
                            ...
                            "transformation_correct": abs(loader_min - (preproc_min * 2.0 - 1.0))
                            < 0.01,
                        }
                    )
                    ...
                    if (
                        abs(loader_min - expected_min) > 0.01
                        or abs(loader_max - expected_max) > 0.01
                    ):
                        results["issues"].append(
...
def validate_file_structure() -> dict[str, any]:
    ...
    results = {"required_files": {}, "required_dirs": {}, "issues": []}
    ...
    for name, path in required_files.items():
        exists = path.exists()
        results["required_files"][name] = {"path": str(path), "exists": exists}
        if not exists:
    ...
    for name, path in required_dirs.items():
        exists = path.exists()
        results["required_dirs"][name] = {"path": str(path), "exists": exists}
        if not exists:
...
def generate_validation_report(results: dict[str, any]) -> None:
    ...
    # save report
    report_path = (
        PATHS["scripts_dir"] / "02_model_development_pipeline" / "pipeline_validation_report.json"
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--skip-preprocessing", action="store_true", help="Skip preprocessing validation"
    )
    parser.add_argument("--skip-dataloader", action="store_true", help="Skip dataloader validation")
    parser.add_argument(
        "--skip-consistency", action="store_true", help="Skip consistency validation"
    )
