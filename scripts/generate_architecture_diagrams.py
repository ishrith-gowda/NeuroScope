...
def create_arrow(ax, start, end, color=COLORS["arrow"], style="->"):
    ...
    arrow = FancyArrowPatch(
        start, end, arrowstyle=style, mutation_scale=15, color=color, linewidth=2
    )
    ax.add_patch(arrow)
    return arrow
...
def generate_generator_architecture():
    ...
    # title
    ax.text(
        8, 8.5, "SA-CycleGAN Generator Architecture", ha="center", fontsize=14, fontweight="bold"
    )
    ...
    for i, (color, label) in enumerate(legend_items):
        rect = plt.Rectangle(
            (0.5 + i * 3.5, 0.3), 0.8, 0.5, facecolor=color, edgecolor="black", alpha=0.8
        )
        ax.add_patch(rect)
...
def generate_attention_mechanism():
    ...
    # title
    ax.text(6, 6.5, "Self-Attention Mechanism", ha="center", fontsize=14, fontweight="bold")
    ...
    ax.text(
        6,
        -0.5,
        r"$\mathrm{SA}(\mathbf{F}) = \gamma \cdot \mathrm{softmax}(\mathbf{Q}\mathbf{K}^\top / \sqrt{d_k})\mathbf{V} + \mathbf{F}$",
        ha="center",
        fontsize=10,
    )
...
def generate_multi_scale_discriminator():
    ...
    # title
    ax.text(
        7, 6.5, "Multi-Scale PatchGAN Discriminator", ha="center", fontsize=14, fontweight="bold"
    )
    ...
    # annotations
    ax.text(6, 0.3, "Receptive Field: 70x70 (PatchGAN)", ha="center", fontsize=9, style="italic")
...
def generate_training_pipeline():
    ...
    # title
    ax.text(8, 10.5, "SA-CycleGAN Training Pipeline", ha="center", fontsize=16, fontweight="bold")
    ...
    ax.text(8, -0.9, "Total Loss = Σ λ_i x L_i", ha="center", fontsize=10, fontweight="bold")
