...
def save_checkpoint(state: dict, filename: str, checkpoint_dir: str, is_best: bool = False):
...
class SACycleGANTrainer:
    ...
    def __init__(self, config: Config):
        ...
        try:
            ...
            print(
                "using advanced losses: perceptual, contrastive, modality-specific, tumor preservation"
            )
    ...
    def train_epoch(
        ...
        for batch_idx in pbar:
            ...
            for d_real, d_fake in zip(d_real_A, d_fake_A):
                ...
                d_loss_A += (
                    self.criterion_gan(d_real, target_real)
                    + self.criterion_gan(d_fake, target_fake)
                ) / 2
    ...
    def train(self, dataloader_A: DataLoader, dataloader_B: DataLoader):
        """full training loop."""
        ...
        # save final models
        torch.save(
            self.model.G_A2B.state_dict(), Path(self.config.checkpoint_dir) / "G_A2B_final.pth"
        )
        torch.save(
            self.model.G_B2A.state_dict(), Path(self.config.checkpoint_dir) / "G_B2A_final.pth"
        )
        torch.save(self.model.D_A.state_dict(), Path(self.config.checkpoint_dir) / "D_A_final.pth")
        torch.save(self.model.D_B.state_dict(), Path(self.config.checkpoint_dir) / "D_B_final.pth")
...
def parse_args():
    ...
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default="./checkpoints/sa_cyclegan",
        help="Directory for checkpoints",
    )
    parser.add_argument(
        "--log_dir", type=str, default="./runs/sa_cyclegan", help="Directory for TensorBoard logs"
    )

    # device
    parser.add_argument(
        "--device", type=str, default="auto", help="Device to use (auto, cuda, mps, cpu)"
    )
