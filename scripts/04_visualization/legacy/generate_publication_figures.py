...
def create_architecture_diagram(save_dir, colors):
    ...
    ax.text(
        5,
        6.5,
        "CycleGAN Architecture for Multi-Modal MRI Domain Adaptation",
        ha="center",
        fontsize=14,
        fontweight="bold",
    )
...
def create_generator_diagram(save_dir, colors):
    ...
    ax.text(
        7,
        3.7,
        "ResNet Generator Architecture (6 Residual Blocks)",
        ha="center",
        fontsize=12,
        fontweight="bold",
    )
...
def create_dataset_statistics(save_dir, colors):
    ...
    colors_mod = [colors["primary"], colors["secondary"], colors["tertiary"], colors["quaternary"]]

    bars = ax.barh(modalities, [1, 1, 1, 1], color=colors_mod, edgecolor="black", alpha=0.8)
    ax.set_xlim(0, 1.5)
    ax.set_xlabel("")
    ax.set_title("(c) MRI Modalities Used")

    for bar, prop in zip(bars, properties):
        ax.text(1.05, bar.get_y() + bar.get_height() / 2, prop, va="center", fontsize=9)
    ax.set_xticks([])
...
def create_techniques_diagram(save_dir, colors):
    ...
    ax.text(
        6,
        5.5,
        "Anti-Mode-Collapse Techniques in CycleGAN Training",
        ha="center",
        fontsize=14,
        fontweight="bold",
    )
    ...
    for _i, (name, desc, color, idx) in enumerate(techniques):
        ...
        # description
        ax.text(x + 1.25, y + 0.4, desc, ha="center", va="center", fontsize=8, color="white")
...
def main():
    ...
    parser.add_argument(
        "--output_dir", type=str, default="figures/publication", help="Output directory for figures"
    )
    parser.add_argument(
        "--loss_file", type=str, default=None, help="Path to training loss JSON file"
    )
