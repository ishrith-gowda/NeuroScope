...
def create_comprehensive_figure(results: dict, output_path: Path):
    ...
    gs = gridspec.GridSpec(3, 4, figure=fig, hspace=0.35, wspace=0.3, height_ratios=[1, 1, 0.6])
    ...
    mmds = [raw_feat.get("mmd", 0), harm_feat.get("mmd", 0), combat.get("combat", {}).get("mmd", 0)]
    ...
    if comp_results:
        ...
        bars = ax8.barh(
            methods,
            times,
            color=[COLORS["sa_cyclegan"]] + [COLORS["neutral"]] * len(comp_results),
            edgecolor="black",
            linewidth=0.5,
        )
...
\quad accuracy $\downarrow$ & """
        + f"{raw.get('accuracy', 0):.3f}"
        + r""" & """
        + f"{harm.get('accuracy', 0):.3f}"
        + r""" & 0.750 & """
        + f"{domain_class.get('improvement', {}).get('accuracy_reduction', 0) * 100:.1f}"
        + r"""\% \\
\quad auc $\downarrow$ & """
        + f"{raw.get('auc', 0):.3f}"
        + r""" & """
        + f"{harm.get('auc', 0):.3f}"
        + r""" & 0.800 & """
        + f"{domain_class.get('improvement', {}).get('auc_reduction', 0) * 100:.1f}"
        + r"""\% \\
\midrule
\multicolumn{5}{l}{\textit{feature distribution}} \\
\quad mmd $\downarrow$ & """
        + f"{raw_feat.get('mmd', 0):.4f}"
        + r""" & """
        + f"{harm_feat.get('mmd', 0):.4f}"
        + r""" & 0.0027 & """
        + f"{comprehensive.get('summary', {}).get('sa_cyclegan_mmd_reduction_percent', 0):.1f}"
        + r"""\% \\
\quad cosine similarity $\uparrow$ & """
        + f"{raw_feat.get('cosine_similarity', 0):.4f}"
        + r""" & """
        + f"{harm_feat.get('cosine_similarity', 0):.4f}"
        + r""" & 1.0000 & +"""
        + f"{(harm_feat.get('cosine_similarity', 0) - raw_feat.get('cosine_similarity', 0)):.4f}"
        + r""" \\
\quad mean difference $\downarrow$ & """
        + f"{raw_feat.get('mean_difference', 0):.4f}"
        + r""" & """
        + f"{harm_feat.get('mean_difference', 0):.4f}"
        + r""" & 0.0126 & """
        + f"{(1 - harm_feat.get('mean_difference', 0) / raw_feat.get('mean_difference', 0)) * 100:.1f}"
        + r"""\% \\
\bottomrule
\end{tabular}
\end{table*}
...
def main():
    ...
    parser.add_argument(
        "--results-dir", type=str, required=True, help="directory containing all evaluation results"
    )
    parser.add_argument(
