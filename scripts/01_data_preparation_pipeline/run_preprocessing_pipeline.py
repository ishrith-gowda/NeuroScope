...
def run_stage(entry, base_args, dry_run=False):
    ...
    stderr_thread = threading.Thread(
        target=print_output, args=(process.stderr, f"{entry['name']}-ERROR")
    )
    ...
    while process.poll() is None:
        ...
        if current_time - last_output_time > 15:  # no output for 15 seconds
            print(
                f"[{entry['name']}] still running... (no output for {int(current_time - last_output_time)} seconds)"
            )
...
def check_outputs(entry):
    ...
    if missing:
        logging.warning(
            f"Stage {entry['name']} missing {len(missing)}/{len(entry['required_outputs'])} expected outputs"
        )
        return False

    if entry["required_outputs"]:
        logging.info(
            f"Stage {entry['name']} verified {len(entry['required_outputs'])} expected outputs +"
        )
    return True
...
def parse_args():
    ...
    ap.add_argument(
        "--stop-on-fail", action="store_true", help="Stop pipeline at first failed stage"
    )
...
def main():
    ...
    logging.info(
        f"Configuration: force={args.force}, dry_run={args.dry_run}, stop_on_fail={args.stop_on_fail}"
    )
    ...
    for idx, stage in enumerate(STAGES):
        ...
        if already_ok and not args.force:
            logging.info(
                f"Skipping stage {stage_num}/{total_stages} '{stage['name']}' (outputs already present)"
            )
        ...
        if not success:
            logging.error(
                f"Stage {stage_num}/{total_stages} '{stage['name']}' FAILED after {time_str}"
            )
            ...
            if produced:
                logging.info(
                    f"Stage {stage_num}/{total_stages} '{stage['name']}' completed successfully in {time_str}"
                )
            else:
                logging.warning(
                    f"Stage {stage_num}/{total_stages} '{stage['name']}' ran without errors but expected outputs not found. Elapsed: {time_str}"
                )
    ...
    logging.info(f"{'-' * 80}")
    logging.info(
        f"SUCCESS: {status_count['ok']} | FAILED: {status_count['failed']} | "
