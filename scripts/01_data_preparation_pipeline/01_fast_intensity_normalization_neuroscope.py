...
def find_modality_paths_fixed(section: str, subj_id: str) -> list[str]:
    ...
    for pattern in expected_patterns:
        ...
        if not matches:
            logging.warning(
                "missing modality %s for %s/%s in files: %s", pattern, section, subj_id, files
            )
            return []
...
def fast_intensity_normalize(
    ...
    try:
        ...
        if brain_voxels.size == 0:
            logging.warning(
                "no brain voxels found after background exclusion, using global normalization"
            )
            ...
            if brain_high_val <= brain_low_val:
                logging.warning(
                    "invalid percentile range: low=%.2f, high=%.2f, using global normalization",
                    brain_low_val,
                    brain_high_val,
                )
                # fallback: use global normalization
                ...
                arr_normalized[brain_mask] = (brain_intensities - brain_low_val) / (
                    brain_high_val - brain_low_val
                )
        ...
        logging.debug(
            "normalization: bg_thresh=%.2f, brain_range=[%.2f, %.2f] → [%.3f, %.3f]",
            background_threshold,
            brain_low_val if "brain_low_val" in locals() else "N/A",
            brain_high_val if "brain_high_val" in locals() else "N/A",
            output_min,
            output_max,
        )
...
def process_subject_fast_intensity_only(
    ...
    for i, path in enumerate(paths):
        if not verify_mri_path(path):
            logging.error(
                "[%s/%s] aborting: input file %d appears to be a mask: %s",
                section,
                subj_id,
                i,
                path,
    ...
    for i, (input_path, mod_name) in enumerate(zip(paths, modality_names)):
        ...
        if not verify_mri_path(output_path):
            logging.error(
                "[%s/%s] error: final output appears corrupted: %s", section, subj_id, mod_name
            )
            return False

        mod_elapsed = time.time() - mod_start
        logging.info(
            "[%s/%s] %s completed in %.1f seconds", section, subj_id, mod_name, mod_elapsed
        )

    total_elapsed = time.time() - start_time
    logging.info(
        "[%s/%s] fast intensity normalization completed in %.1f seconds",
        section,
        subj_id,
        total_elapsed,
    )
...
def process_subjects_from_splits(
    ...
    for idx, (section, sid, split) in enumerate(tasks, 1):
        ...
        if process_subject_fast_intensity_only(
            ...
            logging.info(
                "success: %s/%s (%.1fs) | avg: %.1fs/subject | eta: %.1f min",
                section,
                sid,
                subject_time,
                avg_time,
    ...
    logging.info(
        "total time: %.1f minutes (%.1f seconds/subject avg)",
        elapsed_total / 60,
        total_processing_time / max(success_count, 1),
    )
...
def parse_args():
    ...
    p.add_argument(
        "--splits", type=str, default="train,val,test", help="Comma list of splits to process"
    )
    return p.parse_args()
