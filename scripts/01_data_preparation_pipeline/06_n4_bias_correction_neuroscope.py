...
def parse_args():
    ...
    ap.add_argument(
        "--splits", type=str, default="train,val", help="Comma list of splits to process"
    )
    ap.add_argument("--overwrite", action="store_true")
    ap.add_argument("--max-workers", type=int, default=max(1, mp.cpu_count() - 1))
    ap.add_argument(
        "--bias-threshold", type=float, default=0.18, help="Median slice CV threshold to trigger N4"
    )
    ap.add_argument(
        "--second-pass-iters",
        type=str,
        default="10,5",
        help="Second pass iteration schedule if first conservative attempt rejected",
    )
...
def process_subject_improved(args: Tuple[str, str, bool, float, Tuple[int, int]]) -> Dict:
    ...
    try:
        ...
        if (
            result["processed_modalities"]
            or result["skipped_modalities"]
            or result["skipped_unnecessary"]
        ):
            result["success"] = True
    ...
    for section in ["brats", "upenn"]:
        ...
        for subject_id, subject_info in valid_subjects.items():
            if subject_info.get("split") in splits_to_process:
                ...
                has_valid_files = any(
                    validate_image_file(subject_dir / f"{mod}.nii.gz") for mod in modalities
                )
...
def run_improved_n4_correction(
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        future_to_subject = {
            executor.submit(process_subject_improved, args): args[1] for args in args_list
        }
        ...
        for future in as_completed(future_to_subject):
            try:
                ...
                if result["success"]:
                    ...
                    results["total_modalities_skipped_unnecessary"] += len(
                        result["skipped_unnecessary"]
                    )
                else:
                ...
                if completed % 25 == 0:
                    ...
                    logging.info(
                        f"progress: {completed}/{len(subjects)} subjects "
...
def print_improved_summary(results: Dict):
    ...
    total_modalities = (
        results["total_modalities_processed"]
    ...
    for detail in results["processing_details"]:
        ...
        skipped_unnecessary_count += len(
            [
                x
                for x in detail["skipped_unnecessary"]
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "second_pass": tuple(int(x) for x in parsed.second_pass_iters.split(",") if x.strip())[:2]
        or (10, 5),
    }
    main(config)
