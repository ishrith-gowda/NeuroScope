...
def parse_args():
    ...
    ap.add_argument(
        "--summary-only", action="store_true", help="Skip saving detailed results, only summary"
    )
...
def compare_before_after_n4(
    ...
    metrics = {
        ...
        "cv_improvement_ratio": float(
            np.nanmedian(orig_slice_cvs) / (np.nanmedian(corr_slice_cvs) + 1e-8)
        ),
        "intensity_preservation": float(np.corrcoef(orig_brain, corr_brain)[0, 1])
        if len(orig_brain) > 1
        else np.nan,
    }
...
def assess_n4_effectiveness_for_subject(
    ...
    for modality in modalities:
        ...
        modality_result = {
            "files_status": {"original_exists": orig_exists, "n4_corrected_exists": n4_exists}
        }
        ...
        try:
            ...
            if "cv_improvement_ratio" in comparison_metrics and np.isfinite(
                comparison_metrics["cv_improvement_ratio"]
            ):
    ...
    if processed_modalities > 0:
        subject_results["overall_effectiveness"] = {
            ...
            if np.mean(total_improvements) > 1.2
            else "moderate"
            if np.mean(total_improvements) > 1.1
            else "poor",
        }
    else:
        subject_results["overall_effectiveness"] = {
    ...
    for section in ["brats", "upenn"]:
        ...
        for subject_id, subject_info in valid_subjects.items():
            ...
            try:
                ...
                logging.info(
                    "completed %s/%s (%s) - %d/%d modalities assessed",
                    section,
                    subject_id,
                    subject_split,
        ...
        logging.info(
            "%s assessment complete: %d processed, %d successful",
            section,
            section_processed,
            section_successful,
        )
...
def print_n4_assessment_summary(summary: dict[str, Any], results: dict[str, Any]) -> None:
    ...
    total_assessed = proc_summary.get("successful_subjects", 0) + proc_summary.get(
        "subjects_with_partial_assessment", 0
    )
        ...
        try:
            ...
            for section in ["brats", "upenn"]:
                ...
                for split in ["train", "val", "test"]:
                    ...
                    if total > 0:
                        logging.info(
                            "  %s: %d/%d (%.1f%%) n4-corrected",
                            split,
                            n4_corrected,
                            total,
                            n4_corrected / total * 100,
                        )
