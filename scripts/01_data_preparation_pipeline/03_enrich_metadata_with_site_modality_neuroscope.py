...
def enrich_brats_metadata(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    for _subject_id, subj_info in valid_subjects.items():
        ...
        subj_info["notes"] = (
            "Multi-institutional TCGA dataset with heterogeneous acquisition parameters"
        )
...
def enrich_upenn_metadata(metadata: dict[str, Any], acquisition_df: pd.DataFrame) -> dict[str, Any]:
    ...
    for subject_id, subj_info in valid_subjects.items():
        ...
        if vendor or model or field_strength:
            enriched_count += 1
            logging.debug(
                "enriched %s: vendor=%s, model=%s, field=%.1fT",
                subject_id,
                vendor,
                model,
                field_strength or 0.0,
            )
        else:
            missing_count += 1
...
def print_enrichment_summary(metadata: dict[str, Any]) -> None:
    ...
    for dataset_name, section_key in [("BraTS-TCGA-GBM", "brats"), ("UPenn-GBM", "upenn")]:
        ...
        print(
            f"  with vendor info:         {with_vendor} ({with_vendor / total_subjects * 100:.1f}%)"
        )
        print(
            f"  with scanner model:       {with_model} ({with_model / total_subjects * 100:.1f}%)"
        )
        print(
            f"  with field strength:      {with_field} ({with_field / total_subjects * 100:.1f}%)"
        )

        if section_key == "upenn" and with_field > 0:
            field_strengths = [
                s["field_strength"]
                for s in valid_subjects.values()
