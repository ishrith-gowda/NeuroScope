...
def validate_enriched_metadata(metadata: Dict[str, Any]) -> bool:
    ...
    logging.info(
        "metadata validation passed: %d brats + %d upenn = %d total subjects",
        brats_count,
        upenn_count,
        total_subjects,
    )
...
def stratified_split_subjects(
    ...
    logging.info(
        "split %d subjects: %d train, %d val, %d test (seed=%d)",
        total,
        n_train,
        n_val,
        n_test,
...
def print_split_summary(summary: Dict[str, Dict[str, int]]) -> None:
    ...
    for domain, domain_name in [("brats", "BraTS-TCGA-GBM"), ("upenn", "UPenn-GBM")]:
        ...
        print(f"\n{domain_name}:")
        print(
            f"  train:      {counts['train']:3d} subjects ({counts['train'] / max(total, 1) * 100:.1f}%)"
        )
        print(
            f"  validation: {counts['val']:3d} subjects ({counts['val'] / max(total, 1) * 100:.1f}%)"
        )
        print(
            f"  test:       {counts['test']:3d} subjects ({counts['test'] / max(total, 1) * 100:.1f}%)"
        )
    ...
    print(
        f"  validation: {overall['val']:3d} subjects ({overall['val'] / max(total, 1) * 100:.1f}%)"
    )
    print(
        f"  test:       {overall['test']:3d} subjects ({overall['test'] / max(total, 1) * 100:.1f}%)"
    )
    print(f"  total:      {total:3d} subjects")
...
def main() -> None:
    ...
    try:
        ...
        split_config = {"train_frac": 0.7, "val_frac": 0.15, "test_frac": 0.15, "seed": 42}
        ...
        logging.info(
            "domain assignment and splitting completed successfully in %.2f seconds", elapsed_time
        )
