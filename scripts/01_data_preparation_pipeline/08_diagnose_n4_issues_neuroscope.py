...
def parse_args():
    ...
    ap.add_argument(
        "--max-per-section", type=int, default=15, help="Max subjects per section to diagnose"
    )
...
def diagnose_single_subject(section: str, subject_id: str, modalities: List[str]) -> Dict[str, Any]:
    ...
    for modality in modalities:
        ...
        if (
            not modality_diagnosis["files_exist"]["original"]
            or not modality_diagnosis["files_exist"]["n4_corrected"]
        ):
        ...
        try:
            ...
            modality_diagnosis.update(
                {
                    "intensity_analysis": {"original": orig_stats, "n4_corrected": n4_stats},
                    "uniformity_analysis": {
            ...
            if scaling_analysis.get("extreme_scaling_detected", False):
                issues.append(
                    f"Extreme intensity scaling detected (ratio: {scaling_analysis.get('range_ratio', 'N/A'):.2f})"
                )

            # issue 3: mean intensity shift
            if scaling_analysis.get("mean_shift_detected", False):
                issues.append(
                    f"Large mean intensity shift (ratio: {scaling_analysis.get('mean_ratio', 'N/A'):.2f})"
                )

            # issue 4: poor correlation
            if scaling_analysis.get("poor_correlation_detected", False):
                issues.append(
                    f"Poor correlation between original and N4 ({scaling_analysis.get('correlation', 'N/A'):.3f})"
                )

            # issue 5: negative values
            if scaling_analysis.get("negative_values_count", 0) > 0:
                issues.append(
                    f"Negative values introduced: {scaling_analysis['negative_values_count']}"
                )

            modality_diagnosis["detected_issues"] = issues

            if issues:
                subject_diagnosis["overall_issues"].extend(
                    [f"{modality}: {issue}" for issue in issues]
                )
...
def run_diagnostic_analysis(
    ...
    results = {
        ...
        "subjects_analyzed": {"brats": [], "upenn": []},
        "common_issues": defaultdict(int),
        "detailed_diagnoses": {"brats": {}, "upenn": {}},
    }
    ...
    for section in ["brats", "upenn"]:
        ...
        sample_subjects = random.sample(
            available_subjects, min(max_subjects_per_section, len(available_subjects))
        )
...
def generate_diagnostic_summary(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    summary = {
        "analysis_overview": {
            "subjects_analyzed": sum(
                len(subjects) for subjects in results["subjects_analyzed"].values()
            ),
    ...
    for issue, count in sorted_issues:
        if "CV increased" in issue:
            ...
            recommendations.append(
                "N4 correction is causing extreme intensity scaling - consider more conservative parameters"
            )
        elif "mean intensity shift" in issue:
            recommendations.append(
                "Large mean intensity shifts detected - verify intensity normalization steps"
            )
        elif "Poor correlation" in issue:
            recommendations.append(
                "Poor correlation suggests N4 correction is distorting image content"
            )
        elif "Negative values" in issue:
            recommendations.append(
                "Negative values introduced - check N4 implementation and input image preprocessing"
            )
...
def main():
    ...
    try:
        ...
        results = run_diagnostic_analysis(
            metadata, max_subjects_per_section=args.max_per_section, splits=splits
        )
