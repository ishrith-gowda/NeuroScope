...
def compute_slice_wise_statistics(image: sitk.Image, mask: sitk.Image) -> dict[str, float]:
    ...
    if valid_volumes.sum() > 0:
        ...
        metrics["volume_weighted_variation"] = (
            np.sqrt(weighted_var) / global_std if global_std > 0 else np.nan
        )
    else:
...
def assess_subject_bias(
    ...
    for suffix, _file_path in modality_files.items():
        ...
        preprocessed_path = (
            PATHS["preprocessed_dir"] / section / subject_id / f"{modality_name}.nii.gz"
        )
        ...
        try:
            ...
            logging.debug(
                "%s/%s %s: slice_var=%.4f, trend_slope=%.6f",
                section,
                subject_id,
                modality_name,
...
def analyze_dataset_bias(
    ...
    for section in ["brats", "upenn"]:
        ...
        for subject_id, subject_info in valid_subjects.items():
            ...
            try:
                ...
                if not preprocessed_exist:
                    logging.debug(
                        "no preprocessed files found for %s/%s (split: %s) - skipping",
                        section,
                        subject_id,
                        subject_split,
                    )
                ...
                subject_results = assess_subject_bias(
                    section, subject_id, modality_files, config["names"]
                )
        ...
        logging.info(
            "%s assessment complete: %d successful, %d failed, %d skipped",
            section,
            section_successful,
            section_failed,
            section_skipped,
...
def save_bias_assessment_results(
    ...
    output_data = {
        ...
        "detailed_results": {"brats": results["brats"], "upenn": results["upenn"]},
    ...
    try:
        ...
        logging.info(
            "bias assessment results saved to: %s (%.1f KB)", output_path, file_size / 1024
        )
...
def print_bias_assessment_summary(summary: dict[str, Any], results: dict[str, Any]) -> None:
    ...
    print(
        f"  success rate:        {proc_info.get('successful_subjects', 0) / max(proc_info.get('total_subjects', 1), 1) * 100:.1f}%"
    )
        ...
        for subject_id, subject_info in metadata[section]["valid_subjects"].items():
            ...
            if split in status[section]:
                ...
                for modality in modality_configs[section]:
                    preprocessed_path = (
                        PATHS["preprocessed_dir"] / section / subject_id / f"{modality}.nii.gz"
                    )
                    if verify_preprocessed_file(str(preprocessed_path)):
                        preprocessed = True
                        break
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Comprehensive intensity bias assessment (configurable)"
    )
    ap.add_argument(
        "--splits", type=str, default="train,val", help="Comma list of dataset splits to assess"
    )
    ap.add_argument("--output", type=str, default=None, help="Override output JSON path")
    ap.add_argument(
        "--no-summary", action="store_true", help="Disable writing separate summary JSON"
    )
    ...
    try:
        ...
        for section in ["brats", "upenn"]:
            ...
            for split in ["train", "val", "test"]:
                ...
                if total > 0:
                    logging.info(
                        "  %s: %d/%d (%.1f%%) preprocessed",
                        split,
                        preprocessed,
                        total,
                        preprocessed / total * 100,
                    )
