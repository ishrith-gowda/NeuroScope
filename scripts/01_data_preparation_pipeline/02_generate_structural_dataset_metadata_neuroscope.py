...
def find_modality_files(
    ...
    for suffix in modality_suffixes:
        ...
        if matches:
            ...
            if len(matches) > 1:
                logging.warning(
                    "multiple files found for %s in %s, using: %s",
                    suffix,
                    subject_dir.name,
                    matches[0].name,
                )
...
def scan_dataset(
    ...
    logging.info(
        "%s scan complete: %d valid, %d incomplete subjects", dataset_name, len(valid), len(missing)
    )
...
def main() -> None:
    ...
    for dataset_name, section_key in [("BraTS-TCGA-GBM", "brats"), ("UPenn-GBM", "upenn")]:
        ...
        print(
            f"  completion rate:          {info['complete_subjects'] / max(info['total_subjects'], 1) * 100:.1f}%"
        )

    total_complete = (
        metadata["brats"]["dataset_info"]["complete_subjects"]
