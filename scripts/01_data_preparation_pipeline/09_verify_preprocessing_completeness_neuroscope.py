            ...
            if "error" not in quality_metrics:
                ...
                if quality_metrics["intensity_range"] < 0.1:  # very low range
                    quality_issues.append(
                        f"{modality}: low intensity range ({quality_metrics['intensity_range']:.3f})"
                    )
                ...
                quality_issues.append(
                    f"{modality}: quality analysis failed - {quality_metrics['error']}"
                )
        else:
                ...
                if mod_status.get("file_exists", False):
                    ...
                    if "error" not in quality_metrics:
                        ...
                        if intensity_range is not None and np.isfinite(intensity_range):
                            stats["quality_metrics"]["intensity_ranges"].append(
...
def run_pipeline_verification(
    ...
    for section in ["brats", "upenn"]:
        ...
        section_complete = sum(
            1
            for s in results["detailed_results"][section].values()
            if s.get("overall_complete", False)
        )
        section_training_ready = sum(
            1
            for s in results["detailed_results"][section].values()
            if s.get("ready_for_training", False)
        )

        logging.info(
            "%s verification complete: %d total, %d complete, %d training-ready",
            section,
            section_processed,
            section_complete,
            section_training_ready,
        )
    ...
    quality_analysis = {
        "spacing_compliance": float(
            stats["quality_metrics"]["spacing_compliant"]
            / max(stats["quality_metrics"]["total_modalities_checked"], 1)
        )
    }
...
def generate_verification_summary(results: dict[str, Any]) -> dict[str, Any]:
    ...
    if total_subjects > 0:
        ...
        summary["completion_rates"] = {
            ...
            "subjects_with_issues": float(
                results["summary_statistics"]["subjects_with_quality_issues"] / total_subjects
            ),
        }
    ...
    if total_subjects > 0:
        ...
        if training_ready_rate < 0.90:
            recommendations.append(
                f"Only {training_ready_rate * 100:.1f}% of subjects are training-ready - investigate quality issues"
            )

    if results["summary_statistics"]["missing_files_count"] > 0:
        recommendations.append(
            f"{results['summary_statistics']['missing_files_count']} missing files detected - run preprocessing scripts"
        )

    if cv_stats.get("median", 0) > 0.3:
        recommendations.append(
            "High intensity variation detected - consider additional normalization"
        )
    ...
    if not recommendations:
        recommendations.append(
            "Pipeline verification passed - data is ready for CycleGAN training!"
        )
...
def print_verification_summary(summary: dict[str, Any], results: dict[str, Any]) -> None:
    ...
    if completion_rates:
        ...
        print(
            f"  training ready:             {completion_rates.get('training_ready', 0) * 100:.1f}%"
        )
        print(
            f"  subjects with issues:       {completion_rates.get('subjects_with_issues', 0) * 100:.1f}%"
        )
    ...
    for section in ["brats", "upenn"]:
        ...
        if section_data.get("total", 0) > 0:
            ...
            print(f"    total:                    {total}")
            print(
                f"    complete:                 {complete} ({complete / max(total, 1) * 100:.1f}%)"
            )
            print(
                f"    training-ready:           {training_ready} ({training_ready / max(total, 1) * 100:.1f}%)"
            )
    ...
    for modality, display_name in modality_names.items():
        ...
        if total_expected > 0:
            ...
            print(
                f"  {display_name:6}: {available:3}/{total_expected:3} available ({availability_rate:5.1f}%), "
    ...
    if "spacing_consistency" in quality_assessment:
        ...
        spacing_emoji = {"excellent": "🟢", "good": "🟡", "needs_improvement": "🔴"}.get(
            spacing, "⚪"
        )
...
def parse_args():
    ...
    ap.add_argument(
        "--splits", type=str, default="train,val", help="Comma list of splits to verify"
    )
    ap.add_argument("--output", type=str, default=None, help="Override output JSON path")
    ap.add_argument(
        "--summary-only", action="store_true", help="Write only summary file (skip details)"
    )
