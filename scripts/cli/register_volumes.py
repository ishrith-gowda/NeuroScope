...
def parse_args():
    ...
    parser.add_argument(
        "--fixed-path", type=str, required=True, help="Path to fixed (target) image or directory"
    )
    parser.add_argument(
        "--moving-path", type=str, required=True, help="Path to moving (source) image or directory"
    )
    parser.add_argument(
        "--output-path", type=str, required=True, help="Output directory for registered images"
    )
    parser.add_argument(
        "--file-pattern",
        type=str,
    ...
    parser.add_argument(
        "--fixed-mask-path", type=str, help="Optional path to fixed image mask or directory"
    )
    parser.add_argument(
        "--moving-mask-path", type=str, help="Optional path to moving image mask or directory"
    )
    parser.add_argument(
        "--registration-type",
    ...
    parser.add_argument(
        ...
        help="Optimizer for registration",
    )
    parser.add_argument(
        "--learning-rate", type=float, default=0.1, help="Learning rate for optimizer"
    )
    parser.add_argument("--iterations", type=int, default=100, help="Maximum number of iterations")
    parser.add_argument("--save-transforms", action="store_true", help="Save transformation files")
    parser.add_argument("--output-json", type=str, help="Output JSON file for registration metrics")
