...
def parse_args():
    ...
    parser.add_argument(
        "--input-dir", type=str, required=True, help="Input directory containing MRI volumes"
    )
    parser.add_argument(
        "--output-dir", type=str, required=True, help="Output directory for preprocessed volumes"
    )
    parser.add_argument("--mask-dir", type=str, help="Optional directory containing masks")
    parser.add_argument(
        "--file-pattern", type=str, default="*.nii.gz", help="Glob pattern for input files"
    )
    parser.add_argument(
        "--normalize",
        choices=["minmax", "zscore", "percentile", "histequal", "whitestripe"],
        default="percentile",
        help="Normalization method",
    )
    parser.add_argument(
        "--lower-pct", type=float, default=1.0, help="Lower percentile for percentile normalization"
    )
    parser.add_argument(
        "--upper-pct",
        type=float,
    ...
    parser.add_argument("--crop", type=int, nargs=3, help="Crop size as three integers [x, y, z]")
    parser.add_argument(
        "--crop-method", choices=["center", "random"], default="center", help="Cropping method"
    )
    parser.add_argument(
        "--rescale",
        type=float,
    ...
    parser.add_argument(
        "--target-shape", type=int, nargs=3, help="Target shape for rescaling [x, y, z]"
    )
    parser.add_argument(
        "--output-json", type=str, help="Output JSON file for preprocessing metadata"
    )
...
def main():
    ...
    if args.normalize == "minmax":
        preprocessor.add_step("min_max_normalization", {"target_range": tuple(args.target_range)})
        ...
        preprocessor.add_step("histogram_equalization", {"num_bins": 256})
    elif args.normalize == "whitestripe":
        preprocessor.add_step("white_stripe_normalization", {"target_value": 1.0})

    # add crop step if requested
    if args.crop:
        preprocessor.add_step("crop", {"crop_size": tuple(args.crop), "method": args.crop_method})
