...
def create_slice_comparison_grid(output_path: str, n_subjects: int = 4):
    """
...
def create_feature_distribution(output_path: str):
    ...
    ax = axes[0, 1]
    harmonized = (
        source_features * 0.3 + target_features.mean(axis=0) * 0.7 + np.random.randn(500, 2) * 0.5
    )

    ax.scatter(
        harmonized[:, 0], harmonized[:, 1], alpha=0.5, s=10, label="Harmonized BraTS", color="green"
    )
    ax.scatter(target_features[:, 0], target_features[:, 1], alpha=0.5, s=10, label="UPenn")
    ax.set_xlabel("Feature Dimension 1")
    ax.set_ylabel("Feature Dimension 2")
    ...
    harmonized_dist = 0.45 * np.exp(-((x - 0.38) ** 2) / 0.013) + 0.55 * np.exp(
        -((x - 0.66) ** 2) / 0.017
    )
    ...
    ax.plot(
        x, harmonized_dist / harmonized_dist.max(), label="Harmonized", linewidth=2, linestyle="--"
    )
    ...
    for i, (name, color) in enumerate(
        [("BraTS", "#2E86AB"), ("UPenn", "#A23B72"), ("Harmonized", "#388E3C")]
    ):
        center = np.array([np.cos(i * 2 * np.pi / 3), np.sin(i * 2 * np.pi / 3)]) * (
            2 if i < 2 else 0.5
        )
