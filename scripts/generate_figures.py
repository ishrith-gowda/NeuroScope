...
def create_architecture_diagram(output_path: str):
    ...
    # title
    ax.text(
        7, 7.5, "SA-CycleGAN Generator Architecture", ha="center", fontsize=14, fontweight="bold"
    )
...
def create_training_curves(training_log: dict, output_path: str):
    ...
    ax.plot(
        epochs, training_log.get("d_loss", []), color=COLORS["secondary"], label="Discriminator"
    )
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
...
def create_all_figures(output_dir: str = "figures/generated"):
    ...
    create_statistical_significance_plot(
        pairwise_tests, output_dir / "statistical_significance.pdf"
    )
    create_statistical_significance_plot(
        pairwise_tests, output_dir / "statistical_significance.png"
    )
