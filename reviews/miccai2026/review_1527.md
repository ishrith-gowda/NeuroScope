...
**1. Mischaracterization of prior work (LLaVA-CoT).** The paper criticizes LLaVA-CoT [3] as relying on "prompt engineering rather than learning-driven optimization" and describes its four stages as "summarization, description, reasoning, conclusion." Both characterizations are incorrect. LLaVA-CoT's actual stages are *summary, caption, reasoning, conclusion*, where "caption" specifically means a description of image regions relevant to the question and is optionally skipped in text-only cases — a design decision the SARM paper elides. More importantly, LLaVA-CoT is primarily learning-driven: the authors construct a large dataset (~100K samples) with stage-tagged supervision and perform supervised fine-tuning; they additionally propose stage-level beam search at inference. Framing LLaVA-CoT as "prompt engineering" is misleading and undermines the stated differentiation from SARM. Once corrected, the novelty of SARM's contribution — a three-stage paradigm with a merged caption/description — is substantially reduced.
...
**4. HuatuoGPT-Vision numbers are dramatically lower than published.** Table 3 reports HuatuoGPT-Vision [14] at 46.0 MMMU-Med, 53.0 VQA-RAD, 59.8 SLAKE, 32.0 PMC-VQA, 32.0 Path-VQA. The HuatuoGPT-Vision paper (Chen et al., arXiv:2406.19280, 2024) reports its 34B variant at approximately MMMU-Med ~54.4, VQA-RAD 68.1, SLAKE 76.9, PMC-VQA 58.2, Path-VQA 63.5 — roughly 15–31 points higher across all benchmarks. If the SARM authors evaluated a smaller (7B) variant under a different protocol (open-ended exact-match vs. multiple-choice), this should be disclosed, and the comparison should be made at matched model size. As presented, SARM's claim of outperforming HuatuoGPT-Vision is not credible, because the published HuatuoGPT-Vision numbers are substantially higher than SARM's own Table 2/3 results.

**5. Novelty of the length reward is overstated relative to AALC.** Reference [7] (Li et al., "AALC: Large Language Model Efficient Reasoning via Adaptive Accuracy-Length Control", arXiv:2506.20160, 2025) proposes an accuracy-aware length reward of the form R = Att_acc·R_raw + α·R_len with Att_acc = γ + (1-γ)(1-r_acc) and γ = 0.9. SARM's reward R = 0.9·r + 0.1·ℓ (gated on correctness via "if r > 0, else 0") uses the same γ = 0.9 accuracy weight and the same core principle — gate length shaping on correctness so length is never traded for accuracy. The Gaussian kernel over target length is a modest variation on AALC's formulation, not a new class of method. The paper should cite AALC as the direct precedent for accuracy-gated length shaping and ideally ablate the Gaussian kernel against AALC's original reward. As written, the Correctness-Aware Length Reward reads as a drop-in replacement for an existing technique rather than a novel contribution.
